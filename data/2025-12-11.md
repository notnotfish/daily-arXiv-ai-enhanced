...
TL;DR: Lattice是一个推荐框架，通过模型空间重新设计解决多领域推荐中的数据碎片化和基础设施成本问题，在Meta部署后实现了显著的质量提升和成本节约。


<details>
  <summary>Details</summary>
Motivation: 产品、界面、政策和法规的快速演变给工业级推荐模型部署带来挑战，主要问题是跨领域数据碎片化和不断增长的基础设施成本阻碍了持续的质量改进。

Method: 提出Lattice推荐框架，通过模型空间重新设计扩展多领域多目标学习，结合跨领域知识共享、数据整合、模型统一、蒸馏和系统优化。

Result: 在Meta部署Lattice后，收入驱动指标提升10%，用户满意度提高11.5%，转化率提升6%，同时节省20%的容量。

Conclusion: Lattice框架通过全面的模型空间重新设计，有效解决了工业级推荐系统中的数据碎片化和成本问题，实现了质量和成本效率的双重提升。
...
### [2] [Goal inference with Rao-Blackwellized Particle Filters](https://arxiv.org/abs/2512.09269)
*Yixuan Wang,Dan P. Guralnik,Warren E. Dixon*

Main category: cs.LG

TL;DR: 该论文提出了一种基于Rao-Blackwellized粒子滤波的意图推断方法，用于从噪声轨迹观测中推断移动代理的最终目标，并引入两种估计器，通过信息论指标量化推断性能。


<details>
  <summary>Details</summary>
Motivation: 从噪声轨迹观测中推断移动代理的最终目标是一个基本估计问题。现有方法在样本效率方面存在不足，需要开发更有效的意图推断技术，特别是针对具有可证明稳定性的闭环行为代理。

Method: 使用改进的Rao-Blackwellized粒子滤波器，利用代理动力学的封闭形式，解析地边缘化线性高斯子结构，仅更新粒子权重以提高样本效率。引入两种差异估计器：基于RBPF权重的高斯混合模型和限制在有效样本上的简化版本。

Result: 通过信息论泄漏指标量化对手恢复代理意图的能力，提供Kullback-Leibler散度的可计算下界。实验表明，对于合规代理，该方法能实现快速准确的意图恢复，简化估计器性能接近完整版本。

Conclusion: 提出的RBPF方法在意图推断中表现出色，为未来设计意图混淆控制器提供了理论基础和实用工具，简化估计器在实际应用中具有良好性能。
...
### [3] [Optimizing Algorithms for Mobile Health Interventions with Active Querying Optimization](https://arxiv.org/abs/2512.08950)
*Aseel Rawashdeh*

Main category: cs.LG

TL;DR: 本文提出了一种基于贝叶斯扩展的ATM方法，用卡尔曼滤波式贝叶斯更新替代标准Q学习，在移动健康干预中实现更稳定、样本效率更高的强化学习。


<details>
  <summary>Details</summary>
Motivation: 移动健康干预中的强化学习需要在干预效果和用户负担之间取得平衡，特别是在状态测量（如用户调查或反馈）成本高昂但又至关重要的情况下。标准的ATM启发式方法虽然解决了控制与测量动作分离的问题，但其基于时间差分Q学习的方法在稀疏和嘈杂环境中容易不稳定。

Method: 提出ATM的贝叶斯扩展，用卡尔曼滤波风格的贝叶斯更新替代标准Q学习，维护具有不确定性感知的Q值估计，实现更稳定和样本效率更高的学习。

Result: 在小型表格环境中，贝叶斯ATM实现了相当或改进的标量化回报，方差显著降低，策略行为更稳定。但在更大更复杂的移动健康环境中，标准和贝叶斯ATM变体都表现不佳，表明ATM的建模假设与真实世界移动健康领域结构挑战之间存在不匹配。

Conclusion: 研究结果强调了不确定性感知方法在低数据环境中的价值，同时强调了需要开发新的强化学习算法，这些算法应明确建模因果结构、连续状态以及在观测成本约束下的延迟反馈。
...
### [4] [Learning When to Ask: Simulation-Trained Humanoids for Mental-Health Diagnosis](https://arxiv.org/abs/2512.08952)
*Filippo Cenacchi,Deborah Richards,Longbing Cao*

Main category: cs.LG

TL;DR: 开发了一个虚拟人形机器人对话代理训练系统，通过模拟276个患者访谈数据，使用TD3算法优化对话时机、节奏和信任度，无需硬件负担即可训练社交互动技能。


<details>
  <summary>Details</summary>
Motivation: 传统人形机器人测试存在速度慢、设备磨损、迭代受限等问题，而现有模拟器大多忽略了非语言动态的政策学习，控制器过于关注任务准确性而忽视了信任、节奏和融洽关系等社交因素。

Method: 将人形机器人虚拟化为对话代理，使用276个Unreal Engine MetaHuman患者模型，包含同步语音、注视/面部表情和头躯干姿态。采用感知-融合-政策循环决策何时说话、何时回应、如何避免打断，使用反事实回放（有界非语言扰动）和不确定性感知的轮次管理器来减少诊断模糊性。

Result: 自定义TD3算法在三个控制器比较中表现最佳，优于PPO和CEM，实现了接近完美的覆盖率，节奏更稳定，奖励相当。决策质量分析显示可忽略的轮次重叠、对齐的打断时机、更少的澄清提示和更短的等待时间。性能在模态丢失和渲染器更换下保持稳定。

Conclusion: 提出了一个以代理为中心的模拟器，将访谈转化为交互式患者；一个安全学习循环，将时机和融洽关系作为首要控制变量；比较研究显示TD3在完整性和社交时机方面有明显优势；消融和鲁棒性分析解释了这些优势，为临床医生监督下的人形机器人试点奠定了基础。
...
### [5] [An Electrocardiogram Multi-task Benchmark with Comprehensive Evaluations and Insightful Findings](https://arxiv.org/abs/2512.08954)
...
<details>
  <summary>Details</summary>
Motivation: 心电图分析需要领域专业知识，这限制了人工智能在医疗保健中的应用。虽然自监督学习和基础模型使AI系统能够获取领域知识，但缺乏对基础模型在ECG分析性能上的全面评估。

Method: 研究通过比较语言/通用时间序列/ECG基础模型与时间序列深度学习模型，评估它们在ECG分析任务上的表现，使用公开可用的基准数据集和代码。

Result: 实验结果显示，通用时间序列/ECG基础模型达到了80%的顶级性能率，表明它们在ECG分析中具有有效性。研究提供了深入分析和综合实验结果。

Conclusion: 该研究强调了基础模型在推进生理波形分析方面的局限性和潜力，为ECG分析领域提供了有价值的基准评估。
...
### [6] [LLM4XCE: Large Language Models for Extremely Large-Scale Massive MIMO Channel Estimation](https://arxiv.org/abs/2512.08955)
...
Method: 提出LLM4XCE框架，包含精心设计的嵌入模块和并行特征-空间注意力机制，深度融合导频特征和空间结构，构建语义丰富的LLM输入表示。仅微调顶部两层Transformer层，高效捕获导频数据的潜在依赖关系

Result: 大量仿真表明，LLM4XCE在混合场条件下显著优于现有最先进方法，实现了优越的估计精度和泛化性能

Conclusion: LLM4XCE框架成功将大语言模型的语义建模能力应用于XL-MIMO信道估计，为混合场信道估计提供了有效的解决方案
...
### [7] [DW-KNN: A Transparent Local Classifier Integrating Distance Consistency and Neighbor Reliability](https://arxiv.org/abs/2512.08956)
*Kumarjit Pathak,Karthik K,Sachin Madan,Jitin Kapila*

Main category: cs.LG

TL;DR: DW-KNN是一种改进的K近邻分类器，通过结合指数距离和邻居有效性双重加权，在异构特征空间中提供更可靠、可解释的预测，平均准确率达到0.8988，仅次于集成KNN方法。


<details>
  <summary>Details</summary>
Motivation: 传统KNN及其变体假设所有k个邻居具有同等可靠性，这在异构特征空间中成为限制因素，影响了预测的可靠性。需要一种能够区分邻居质量、抑制噪声样本的方法。

Method: 提出DW-KNN（双重加权KNN），整合指数距离加权和邻居有效性加权。该方法具有实例级可解释性，能够抑制噪声或错误标记样本，并降低超参数敏感性。

Result: 在9个数据集上的综合评估显示，DW-KNN平均准确率达到0.8988，在6种方法中排名第2，与最佳性能的集成KNN相差仅0.2%。具有最低的交叉验证方差（0.0156），表明预测稳定性可靠。统计显著性测试证实相对于紧凑性加权KNN提升4.09%，相对于核加权KNN提升1.13%。
...
### [8] [LUMOS: Large User MOdels for User Behavior Prediction](https://arxiv.org/abs/2512.08957)
*Dhruv Nigam*

Main category: cs.LG

TL;DR: LUMOS是一个基于Transformer的架构，通过联合学习多个任务，仅使用原始用户活动数据，消除了特定任务模型和手动特征工程的需求。


<details>
  <summary>Details</summary>
Motivation: 在线B2C平台的用户行为预测面临规模化挑战，传统方法依赖特定任务模型和领域特征工程，耗时、计算成本高、需要专业知识且不可扩展。

Method: LUMOS采用Transformer架构，引入新颖的交叉注意力机制，使预测能够基于未来已知事件（如节假日、促销等），并采用多模态标记化，将用户交易、事件上下文和静态用户人口统计属性通过专门的嵌入路径处理。

Result: 在包含2750亿用户活动标记、2.5亿用户的生产数据集上，LUMOS在5个任务上相比传统特定任务模型表现更优：二分类任务ROC-AUC平均提升0.025，回归任务MAPE降低4.6%。在线A/B测试验证了3.15%的日活跃用户增长。

Conclusion: LUMOS通过消除特定任务模型和手动特征工程，实现了可扩展的用户行为预测，其交叉注意力机制能够捕捉未来事件对用户行为的影响，在实际业务中产生了可衡量的积极影响。
...
### [9] [EEG-Bench: A Benchmark for EEG Foundation Models in Clinical Applications](https://arxiv.org/abs/2512.08959)
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏统一的基准测试框架来评估EEG基础模型在临床应用中的表现，需要标准化的评估协议来比较传统方法和现代基础模型的性能。

Method: 构建了一个统一的基准测试框架，涵盖11个诊断任务和14个公开EEG数据集，采用最小预处理和标准化评估协议，支持传统基线模型和现代基础模型的并行比较。

Result: 基础模型在某些场景下表现良好，但简单模型仍然具有竞争力，特别是在临床分布偏移的情况下。

Conclusion: 该框架为EEG基础模型的临床评估提供了标准化工具，促进了可重复性和应用推广，同时揭示了简单模型在临床环境中的持续价值。
...
### [10] [Resolving Conflicts in Lifelong Learning via Aligning Updates in Subspaces](https://arxiv.org/abs/2512.08960)
...
<details>
  <summary>Details</summary>
Motivation: LoRA在持续学习中存在灾难性遗忘问题，主要原因是新任务梯度与历史权重轨迹之间的对抗性方向更新导致破坏性干扰。
...
Conclusion: PS-LoRA通过解决LoRA中的对抗性方向冲突问题，有效缓解了持续学习中的灾难性遗忘，实现了参数稳定性和任务适应性之间的平衡。
...
TL;DR: 提出SEA攻击方法，利用谱嵌入识别图结构中的脆弱方向，通过删除最稳健的边或添加最不兼容的边来攻击GNN模型


<details>
  <summary>Details</summary>
Motivation: 现有图结构攻击方法大多依赖梯度启发式或局部连通性模式，将边视为同等重要的候选扰动对象，缺乏对图谱特性的系统性利用

Method: 提出谱边攻击(SEA)：1)计算谱嵌入捕捉输入流形的最脆弱方向；2)为每条边或非边分配稳健性分数；3)开发两种攻击变体：谱引导删除攻击（移除最稳健的边）和谱引导添加攻击（在谱空间最不兼容的节点间添加边）

Result: 在基准测试中验证了SEA攻击的有效性，该方法无需梯度，可即插即用到现有GNN架构

Conclusion: SEA提供了一种基于谱分析的图结构攻击新范式，通过系统性利用图的谱特性来指导对抗性扰动，提高了攻击效果
...
### [12] [Financial Instruction Following Evaluation (FIFE)](https://arxiv.org/abs/2512.08965)
...
<details>
  <summary>Details</summary>
Motivation: 语言模型在处理复杂、相互依赖的指令方面存在困难，特别是在金融等高精度关键领域，需要评估模型在金融分析任务中的指令遵循能力。

Method: 引入FIFE基准测试，包含88个人工编写的提示，采用可链式验证的约束系统提供细粒度奖励信号，在零样本设置下评估了53个模型（专有、开源权重、开源）。

Result: 性能层次明显：顶级开源权重模型（严格76.1/宽松79.5）优于领先的专有系统（严格65.9/宽松70.5），而最佳开源模型显著落后（严格45.5/宽松48.9）。所有模型都难以完全满足FIFE的复杂要求。

Conclusion: 即使表现最佳的模型也难以完全满足金融分析任务的复杂指令要求，FIFE基准测试作为开源资源发布，旨在促进金融领域强化学习研究。
...
### [13] [CluCERT: Certifying LLM Robustness via Clustering-Guided Denoising Smoothing](https://arxiv.org/abs/2512.08967)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型能力强大，但仍容易受到对抗攻击，即使是保留语义的同义词替换也可能导致错误预测。现有认证方法存在两个关键限制：1) 由于缺乏对扰动输出的语义验证，导致鲁棒性边界松散；2) 重复采样导致计算成本高

Method: 提出CluCERT框架，采用聚类引导的去噪平滑方法。包含语义聚类过滤器减少噪声样本并保留有意义的扰动，通过精炼模块提取核心语义，以及快速同义词替换策略加速去噪过程
...
### [14] [StructuredDNA: A Bio-Physical Framework for Energy-Aware Transformer Routing](https://arxiv.org/abs/2512.08968)
*Mustapha Hamdi*
...
<details>
  <summary>Details</summary>
Motivation: 大规模计算模型的快速扩展导致能源和计算成本急剧增加，受生物系统中结构和功能从低能量配置中涌现的启发，需要开发能量感知的稀疏架构

Method: 引入StructuredDNA稀疏架构框架，用基于语义能量最小化的生物物理能量引导路由层替代密集的Mixture-of-Experts路由，将输入动态分组为语义密码子，通过最小化结合内聚性、不确定性和计算成本的全局能量函数来选择单个专家
...
### [15] [Learning Robust Representations for Malicious Content Detection via Contrastive Sampling and Uncertainty Estimation](https://arxiv.org/abs/2512.08969)
*Elias Hossain,Umesh Biswas,Charan Gudla,Sai Phani Parsa*

Main category: cs.LG

TL;DR: UCF是一个正例-未标记学习框架，通过不确定性感知对比损失、自适应温度缩放和自注意力LSTM编码器，在噪声和不平衡条件下改善分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决在噪声和不平衡条件下的分类问题，特别是在恶意内容分类等高风险领域，需要更鲁棒和可扩展的PU学习解决方案。

Method: 提出不确定性对比框架（UCF），整合不确定性感知对比损失、自适应温度缩放和自注意力引导的LSTM编码器，动态调整对比权重，使用正例锚点稳定训练，并适应批次级变异性。

Result: 在恶意内容分类任务中，UCF生成的嵌入使多个传统分类器达到93.38%以上的准确率，精度超过0.93，接近完美的召回率，假阴性最少，ROC-AUC分数具有竞争力。可视化分析确认正例和未标记实例之间的清晰分离。

Conclusion: UCF是一个鲁棒且可扩展的PU学习解决方案，适用于网络安全和生物医学文本挖掘等高风险领域，能够产生校准的、有区分度的嵌入。
...
### [16] [Peek-a-Boo Reasoning: Contrastive Region Masking in MLLMs](https://arxiv.org/abs/2512.08976)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法局限于最终答案或注意力图分析，无法提供因果、步骤级别的归因，需要一种能评估多模态模型推理忠实性和稳健性的诊断工具。

Method: CRM通过系统性地掩码标注的视觉区域，对比掩码前后模型的推理轨迹，分析模型在思维链推理各步骤中对视觉区域的依赖模式。

Result: 在VisArgs等数据集上，CRM揭示了两种失败模式：一些模型保持推理结构但在证据缺失时产生幻觉；另一些模型紧密依赖视觉线索但在扰动下崩溃。

Conclusion: CRM将视觉基准重构为诊断工具，强调需要评估多模态模型不仅关注性能，更要关注推理的稳健性和忠实性，推动更全面的评估框架发展。
...
### [17] [Graph Deep Learning for Intracranial Aneurysm Blood Flow Simulation and Risk Assessment](https://arxiv.org/abs/2512.09013)
...
TL;DR: 提出一种图神经网络替代模型，可在不到一分钟内从血管几何结构直接生成全视野血流动力学，实现颅内动脉瘤的实时床边分析。


<details>
  <summary>Details</summary>
Motivation: 颅内动脉瘤破裂风险与局部血流动力学密切相关，但传统计算流体动力学模拟速度慢且需要专业知识，而4D Flow MRI等临床成像方法空间分辨率不足且成本高昂，无法捕捉驱动内皮重塑和破裂风险的精细剪切模式。

Method: 使用图神经网络替代模型，结合图变换器和自回归预测，在患者特异性动脉瘤的高保真模拟综合数据集上进行训练，直接从血管几何结构准确模拟血流、壁面剪切应力和振荡剪切指数。

Result: 模型能够泛化到未见过的患者几何结构和流入条件，无需网格特定校准，可在每个心动周期不到一分钟的时间内重现全视野血流动力学，实现近实时推理并与现有成像管道集成。

Conclusion: 该工作将高保真模拟从专家专用研究工具转变为可部署的数据驱动决策支持系统，无需计算专家即可在患者成像后几分钟内提供高分辨率血流动力学预测，标志着向实时床边动脉瘤分析的转变。
...
### [18] [Improving Multi-Class Calibration through Normalization-Aware Isotonic Techniques](https://arxiv.org/abs/2512.09054)
*Alon Arad,Saharon Rosset*

Main category: cs.LG

TL;DR: 提出新的多类校准方法，通过考虑概率归一化来改进等渗回归在多类问题上的性能
...
Method: 提出两种归一化感知的等渗回归技术：NA-FIR（将归一化直接纳入优化过程）和SCIR（将问题建模为累积双变量等渗回归）

Result: 在多种文本和图像分类数据集上，新方法在负对数似然和期望校准误差指标上持续改进

Conclusion: 提出的归一化感知等渗回归技术为多类校准提供了有效解决方案，优于现有方法
...
### [19] [A Diffusion-Based Framework for High-Resolution Precipitation Forecasting over CONUS](https://arxiv.org/abs/2512.09059)
*Marina Vicens-Miquel,Amy McGovern,Aaron J. Hill,Efi Foufoula-Georgiou,Clement Guilloteau,Samuel S. P. Shen*

Main category: cs.LG

TL;DR: 该研究提出了一种基于扩散的深度学习框架，系统比较了三种残差预测策略，用于高分辨率降水预报，在1-12小时预报范围内显著优于传统数值天气预报系统。


<details>
  <summary>Details</summary>
Motivation: 准确的降水预报对于水文气象风险管理至关重要，特别是对于可能导致山洪暴发和基础设施损坏的极端降雨。传统数值天气预报系统存在局限性，需要提高预报精度和可靠性。

Method: 引入基于扩散的深度学习框架，系统比较三种残差预测策略：1）纯数据驱动模型（仅使用MRMS观测数据）；2）校正模型（仅使用HRRR预报数据）；3）混合模型（整合MRMS和HRRR变量）。在1公里空间分辨率下进行1-12小时自回归预报。

Result: 在所有预报时效上，深度学习框架在像素级和空间统计指标上均优于HRRR基准。混合模型在最短预报时效表现最佳，而HRRR校正模型在较长预报时效（可达12小时）表现最优。研究还提供了针对残差学习设置的校准不确定性量化。
...
### [20] [Contrast transfer functions help quantify neural network out-of-distribution generalization in HRTEM](https://arxiv.org/abs/2512.09067)
*Luis Rangel DaCosta,Mary C. Scott*

Main category: cs.LG

TL;DR: 该研究通过模拟数据系统探究神经网络在HRTEM纳米颗粒分割任务中的分布外泛化能力，发现模型性能随成像条件偏离训练分布而平滑下降。


<details>
  <summary>Details</summary>
Motivation: 神经网络在分布外（OOD）泛化方面的表现不佳，这限制了其在实验工作流程中的成功部署，特别是在实验条件变化较大或真实知识难以建立的情况下。理解神经网络的OOD泛化行为对于其在科学任务中的可靠应用至关重要。

Method: 使用随机结构采样和多层切片模拟生成合成数据，训练和测试超过12,000个神经网络分割模型。通过HRTEM对比传递函数开发了一个框架来比较HRTEM数据集的信息内容并量化OOD域偏移。

Result: 神经网络分割模型在成像条件偏离训练分布时表现出显著的性能稳定性，但性能会平滑且可预测地下降。研究量化了成像条件变化对模型性能的影响。

Conclusion: 虽然神经网络在HRTEM纳米颗粒分割任务中表现出一定的OOD泛化稳定性，但性能会随成像条件偏离训练分布而逐渐下降。该方法在解释原子结构等其他OOD偏移方面存在局限性，需要补充技术来理解这些场景下的泛化行为。
...
### [21] [Modular Deep-Learning-Based Early Warning System for Deadly Heatwave Prediction](https://arxiv.org/abs/2512.09074)
*Shangqing Xu,Zhiyuan Zhao,Megha Sharma,José María Martín-Olalla,Alexander Rodríguez,Gregory A. Wellenius,B. Aditya Prakash*

Main category: cs.LG

TL;DR: DeepTherm是一个无需热相关死亡率历史数据的模块化致命热浪预警系统，使用深度学习双预测管道分离基线死亡率，在西班牙真实数据上表现出色。
...
Method: DeepTherm采用模块化设计，利用深度学习的灵活性，采用双预测管道，从全因死亡率中分离出无热浪和其他异常事件时的基线死亡率，无需热相关死亡率历史数据。

Result: 在西班牙真实数据上的评估显示，DeepTherm在不同地区、时间段和人群组中表现出一致、鲁棒和准确的性能，同时允许在漏报和误报之间进行权衡。
...
### [22] [Beyond the Hype: Comparing Lightweight and Deep Learning Models for Air Quality Forecasting](https://arxiv.org/abs/2512.09076)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan*

Main category: cs.LG

TL;DR: 该研究比较了轻量级加法模型（Facebook Prophet和NeuralProphet）与复杂深度学习模型在北京市PM2.5和PM10污染预测中的性能，发现Facebook Prophet在保持可解释性的同时取得了最佳预测效果。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习和混合模型在空气污染预测研究中占主导地位，但其复杂性和有限的可解释性阻碍了实际应用。本研究旨在探索轻量级加法模型是否能在保持可解释性的同时提供有竞争力的预测性能。

Method: 使用多年污染物和气象数据，应用系统特征选择（相关性、互信息、mRMR）、防泄漏缩放和时序数据分割。训练Facebook Prophet和NeuralProphet模型，并作为对比基准实现了两个机器学习模型（LSTM、LightGBM）和一个传统统计模型（SARIMAX）。

Result: Facebook Prophet在7天保留测试集上表现最佳，对两种污染物的测试R²均超过0.94，优于NeuralProphet、SARIMAX和基于学习的基准模型。

Conclusion: 可解释的加法模型在空气污染预测中仍具有竞争力，在准确性、透明度和部署便利性之间提供了实用的平衡，为实际应用提供了有价值的替代方案。
...
TL;DR: GS-KAN是一种轻量化的KAN架构，通过每层共享一个父函数并应用可学习线性变换来构建边缘函数，解决了传统KAN参数效率低的问题，在高维任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统Kolmogorov-Arnold网络（KANs）虽然具有强大的逼近能力，但由于需要为每个网络边缘维护独特的参数化，导致参数效率低下，限制了其在高维场景下的应用。

Method: 提出GS-KAN架构，受David Sprecher对叠加定理的改进启发，通过每层共享一个可学习的父函数，并对该函数应用可学习的线性变换来构建独特的边缘函数，从而显著减少参数数量。

Result: 在连续函数逼近任务中，GS-KAN优于MLPs和标准KAN基线，同时保持卓越的参数效率；在表格数据回归任务中与现有KAN架构竞争性相当；在高维分类任务中优于MLPs。

Conclusion: GS-KAN实现了KAN架构在严格参数约束下的高维部署，解决了传统实现因参数爆炸而不可行的问题，为KAN在资源受限环境中的应用开辟了新途径。
...
TL;DR: 本文提出了一个统一的认证鲁棒归因框架，通过引入自然Wasserstein度量消除深度网络中的谱放大效应，显著提升归因稳定性，首次为神经网络归因提供了非空认证边界。


<details>
  <summary>Details</summary>
Motivation: 现有数据归因方法对分布扰动敏感，实际可靠性不足。深度网络中的谱放大效应使欧几里得认证变得无效，需要新的度量方法来稳定归因估计。

Method: 1) 凸模型：推导具有可证明覆盖保证的Wasserstein鲁棒影响函数(W-RIF)；2) 深度网络：提出自然Wasserstein度量，在模型自身特征协方差诱导的几何中测量扰动，消除谱放大效应；3) 开发Natural W-TRAK方法。

Result: 1) 自然Wasserstein度量将最坏情况敏感性降低76倍；2) 在CIFAR-10和ResNet-18上，Natural W-TRAK认证了68.7%的排序对，而欧几里得基线为0%；3) 自影响项在标签噪声检测中达到0.970 AUROC，仅检查前20%训练数据即可识别94.1%的损坏标签。

Conclusion: 通过自然Wasserstein度量消除谱放大效应，首次为神经网络归因提供了实用的认证鲁棒性框架，显著提升了归因稳定性和可靠性，并为基于杠杆的异常检测提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在推理效率方面有优势，但现有的启发式采样策略（如置信度阈值）需要手动调优，且在大缓冲区下性能下降。需要更自动化的采样方法。

Method: 将掩码扩散采样形式化为马尔可夫决策过程，使用基于单层Transformer的轻量级策略网络，将dLLM的token置信度映射到解掩码决策，通过强化学习训练采样策略。

Result: 训练的策略在半自回归生成中与最先进启发式方法性能相当，在全扩散设置中表现更优。策略能够迁移到新的底层dLLM和更长序列长度，但在域外数据上性能下降，且精度-效率权衡的细粒度调优具有挑战性。

Conclusion: 强化学习训练的采样策略是扩散语言模型的有效替代方案，能够自动学习采样决策，在保持质量的同时提高效率，但需要进一步解决域外泛化和细粒度调优的问题。
...
TL;DR: SEDONet是一种新型DeepONet变体，使用固定Chebyshev谱字典驱动主干网络，专门针对有界域上的非周期PDE特征设计，相比传统DeepONet和Fourier变体在精度上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统DeepONet的主干网络基于全连接层处理原始坐标，难以有效表示有界域上PDE中常见的尖锐梯度、边界层和非周期结构，特别是在Dirichlet或Neumann边界条件下。

Method: 提出SEDONet（谱嵌入DeepONet），用固定的Chebyshev谱字典替代原始坐标输入来驱动主干网络，为有界域提供专门的归纳偏置，能够更好地捕捉非周期特征。

Result: 在2D泊松、1D Burgers、1D对流扩散、Allen-Cahn动力学和Lorenz-96混沌系统等多个PDE基准测试中，SEDONet相比DeepONet、FEDONet等变体始终获得最低的相对L2误差，平均提升30-40%，在非周期几何上优于Fourier嵌入变体。

Conclusion: SEDONet通过简单的参数中性修改，为有界域上的PDE代理建模提供了一个鲁棒高效的谱框架，Chebyshev嵌入在非周期算子学习中具有重要价值，能更准确地保持高频和边界局部化特征。
...
TL;DR: 本文通过图神经网络理论视角研究Transformer的失败模式，将深度学习视为可学习的信息混合与传播过程，分析解码器Transformer因果特性导致的信息传播瓶颈，并统一现有解决方案的理论基础。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在现代大语言模型中表现出色，但仍存在令人惊讶的失败模式和可预测的不对称性能退化。目前对这些失败模式缺乏理论理解，现有解决方案多为临时性且基于直觉而非理论基础。本文旨在通过GNN理论视角填补这一理论空白。

Method: 采用图神经网络理论框架分析Transformer：1) 将深度学习视为可学习的信息混合与传播过程；2) 利用GNN中已有的信息传播瓶颈理论分析Transformer失败模式；3) 特别分析解码器Transformer因果特性导致的信息传播几何特性；4) 将现有解决方案统一到理论框架下。

Result: 研究发现：1) GNN面临的许多问题同样出现在Transformer中；2) 解码器Transformer的因果特性创造了信息传播的有趣几何特性，导致可预测且可能灾难性的失败模式；3) 现有Transformer解决方案大多缺乏理论基础，但可以在GNN理论框架下得到统一解释。

Conclusion: 本文成功建立了Transformer失败模式与GNN理论之间的桥梁，为理解Transformer失败模式提供了理论框架，解释了现有解决方案的工作原理，并指出了针对特定失败模式的改进方向。这有助于推动从直觉驱动到理论驱动的Transformer研究转变。
...
TL;DR: 提出基于数据驱动的临床决策工具，通过整合多国患者数据和多模态信息，为经导管主动脉瓣置换术（TAVR）选择最优瓣膜类型，以降低永久起搏器植入风险。


<details>
  <summary>Details</summary>
Motivation: TAVR已成为治疗严重主动脉瓣狭窄的微创方法，但不同经导管心脏瓣膜（THV）的选择尚无明确指南，且永久起搏器植入（PPI）是主要术后并发症。需要数据驱动的个性化决策工具来优化瓣膜选择。

Method: 整合美国和希腊患者数据，结合人口统计学、CT扫描和超声心动图三种数据源，统一两国记录系统差异。采用叶级分析利用人群异质性，避免基于不确定反事实风险估计的基准比较。

Result: 最终处方模型在美国内部人群中降低PPI率26%，在希腊外部验证队列中降低16%。这是首个统一的、个性化的TAVR中THV选择处方策略。

Conclusion: 该研究开发的数据驱动临床支持工具能有效降低TAVR术后永久起搏器植入风险，为个性化瓣膜选择提供了首个统一策略，具有重要的临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs和Transformer架构在各种自然语言任务中表现出色，但它们在真实工程领域（特别是模拟电路设计）的可靠性和鲁棒性尚未得到充分探索，这限制了它们在实际工作流程中的应用价值。

Method: 研究调查了不同数据表示对模型行为的影响，比较了小型模型（如T5、GPT-2）与大型基础模型（如Mistral-7B、GPT-oss-20B）在不同训练条件下的表现，重点关注人类在环的AI辅助设计场景。

Result: 研究发现了几个关键可靠性挑战：1）对数据格式敏感；2）生成设计不稳定；3）对未见电路配置的泛化能力有限。这些发现揭示了LLMs在复杂工程任务中的局限性。

Conclusion: 该研究为LLMs作为增强人类在复杂工程任务中能力的工具提供了早期证据，指出了其局限性和潜力，并为设计可靠、可部署的基础模型提供了见解，特别是在结构化、真实世界的应用中。
...
### [30] [Tensor-Compressed and Fully-Quantized Training of Neural PDE Solvers](https://arxiv.org/abs/2512.09202)
*Jinming Lu,Jiayi Tian,Yequan Zhao,Hai Li,Zheng Zhang*

Main category: cs.LG

TL;DR: 提出一个用于边缘设备上高效训练物理信息神经网络(PINNs)的框架，通过量化训练、Stein估计器残差计算和张量分解实现计算和能耗的大幅优化。


<details>
  <summary>Details</summary>
Motivation: PINNs在资源受限的边缘设备上部署面临计算和内存开销大的挑战，主要源于高阶自动微分、密集张量操作和全精度运算，需要开发高效的边缘计算解决方案。

Method: 提出集成全量化训练、Stein估计器残差计算和TT分解权重压缩的框架，包含混合精度训练方法、Stein估计器的差分量化方案和TT层的部分重构方案，并设计PINTA硬件加速器。

Result: 在2-D泊松方程、20-D HJB方程和100-D热方程上，该框架达到与全精度基准相当或更好的精度，同时实现5.5-83.5倍加速和159.6-2324.1倍能耗节省。

Conclusion: 该工作实现了边缘设备上的实时PDE求解，为大规模能效科学计算铺平了道路，解决了PINNs在资源受限平台上的部署难题。
...
### [31] [Contrastive Learning for Semi-Supervised Deep Regression with Generalized Ordinal Rankings from Spectral Seriation](https://arxiv.org/abs/2512.09267)
*Ce Wang,Weihang Dai,Hanru Bai,Xiaomeng Li*

Main category: cs.LG

TL;DR: 提出了一种用于半监督回归的对比学习方法，通过构建特征相似性矩阵并利用谱排序算法恢复未标记样本的序数关系，减少对标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法高度依赖标签信息来恢复特征的序数关系，限制了在半监督回归中的应用。需要减少对昂贵标注的依赖，同时利用未标记数据提升回归模型的表示能力。

Method: 1) 在小批量中同时使用标记和未标记样本构建特征相似性矩阵；2) 通过谱排序算法恢复未标记样本的序数排名；3) 利用标记样本提供正则化指导；4) 使用动态规划算法选择鲁棒特征；5) 将恢复的序数关系用于未标记样本的对比学习，并作为额外的监督信号。

Result: 在多个数据集上的实验验证表明，该方法能够超越现有的最先进半监督深度回归方法，提供了理论保证和实证验证。

Conclusion: 该方法成功地将对比学习扩展到半监督回归场景，通过利用未标记数据恢复序数关系，显著减少了标注依赖，提升了回归模型的鲁棒性和性能。
...
### [32] [Hetero-SplitEE: Split Learning of Neural Networks with Early Exits for Heterogeneous IoT Devices](https://arxiv.org/abs/2512.09313)
*Yuki Oda,Yuta Ono,Hiroshi Nakamura,Hideki Takase*

Main category: cs.LG

TL;DR: 提出Hetero-SplitEE方法，通过异构早期退出和分层训练，让不同计算能力的IoT设备选择不同的分割点协同训练共享神经网络，支持异构设备部署。


<details>
  <summary>Details</summary>
Motivation: 现有分割学习方法假设客户端同质性和统一分割点，这限制了在计算资源异构的真实IoT系统中的适用性。需要解决异构设备协同训练的问题。

Method: 提出Hetero-SplitEE方法，集成异构早期退出到分层训练中，让每个客户端根据计算能力选择不同的分割点。提出两种协同训练策略：顺序策略（顺序训练，共享服务器模型）和平均策略（并行训练，周期性跨层聚合）。

Result: 在CIFAR-10、CIFAR-100和STL-10数据集上使用ResNet-18进行实验，证明该方法在保持竞争力的准确率的同时，能有效支持不同的计算约束。

Conclusion: Hetero-SplitEE方法实现了异构IoT设备协同训练，支持实际部署，为异构IoT生态系统中的协作深度学习提供了实用解决方案。
...
### [33] [Branching Strategies Based on Subgraph GNNs: A Study on Theoretical Promise versus Practical Reality](https://arxiv.org/abs/2512.09355)
*Junru Zhou,Yicheng Wang,Pan Li*

Main category: cs.LG

TL;DR: 该论文研究了子图GNN作为混合整数线性规划分支选择的理论中间方案，证明了节点锚定子图GNN（表达能力低于3-WL）足以近似强分支分数，但实证发现其计算开销导致实际性能不如MPNN和启发式方法。


<details>
  <summary>Details</summary>
Motivation: 现有GNN方法在MILP分支选择中存在表达能力和计算效率的权衡：标准MPNN效率高但表达能力不足，高阶GNN表达能力强但计算成本过高，需要寻找理论中间方案。

Method: 采用节点锚定子图GNN作为理论中间方案，证明其表达能力虽低于3-WL但仍能近似强分支分数，并在四个基准数据集上进行实证评估。

Result: 理论证明节点锚定子图GNN足以近似强分支分数，但实证显示其O(n)复杂度导致内存瓶颈和求解时间增加，实际性能不如MPNN和启发式方法。

Conclusion: 对于MILP分支选择，表达性GNN的计算成本目前超过了决策质量提升带来的收益，未来研究应聚焦于保持效率的表达性提升方法。
...
### [34] [Self-Supervised Learning with Gaussian Processes](https://arxiv.org/abs/2512.09322)
*Yunshan Duan,Sinead Williamson*

Main category: cs.LG

TL;DR: 提出了一种基于高斯过程的自监督学习方法GPSSL，解决了传统自监督学习中正样本生成困难和不确定性量化不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统自监督学习方法需要生成相似观测对，这在某些数据类型中具有挑战性，且缺乏不确定性量化能力，在样本外预测中表现不佳。

Method: 使用高斯过程模型进行表示学习，在表示上施加GP先验，通过最小化鼓励信息性表示的损失函数获得广义贝叶斯后验。GP的协方差函数自然地拉近相似单元的表示，替代了显式定义的正样本。

Result: GPSSL与核PCA和VICReg密切相关，但允许后验不确定性传播到下游任务。在多个数据集上的分类和回归实验表明，GPSSL在准确性、不确定性量化和误差控制方面优于传统方法。

Conclusion: GPSSL提供了一种无需显式正样本生成的自监督学习框架，具有更好的不确定性量化能力，在样本外预测中表现更优。
...
### [35] [Towards Resilient Transportation: A Conditional Transformer for Accident-Informed Traffic Forecasting](https://arxiv.org/abs/2512.09398)
*Hongjun Wang,Jiawei Yong,Jiawei Wang,Shintaro Fukushima,Renhe Jiang*

Main category: cs.LG

TL;DR: 提出ConFormer（条件Transformer）框架，整合交通事故和法规数据，通过图传播和引导归一化层动态调整时空节点关系，在东京和加州数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 交通预测面临外部因素（如交通事故、法规）复杂影响的挑战，现有模型因数据整合有限而忽视这些因素，导致预测准确性受限。

Method: 1）构建包含交通事故和法规数据的东京和加州交通数据集；2）提出ConFormer框架，结合图传播和引导归一化层，基于历史模式动态调整时空节点关系。

Result: ConFormer在预测性能和效率上均超越最先进的STAEFormer，计算成本更低、参数需求更少，在多个指标上持续优于主流时空基线模型。

Conclusion: ConFormer通过有效整合外部因素和动态调整时空关系，显著提升了交通预测准确性，展示了在交通预测研究中的潜力。
...
### [36] [Self Distillation Fine-Tuning of Protein Language Models Improves Versatility in Protein Design](https://arxiv.org/abs/2512.09329)
*Amin Tavakoli,Raswanth Murugan,Ozan Gokdemir,Arvind Ramanathan,Frances Arnold,Anima Anandkumar*

Main category: cs.LG

TL;DR: 提出了一种用于蛋白质语言模型快速监督微调的通用方法，通过轻量级筛选管道构建高质量训练数据，无需昂贵的实验数据集，能生成更稳定、功能性更强且新颖的蛋白质序列。
...
Result: 在色氨酸合酶酶家族上应用基因组规模蛋白质语言模型（GenSLM），微调后模型生成的序列不仅更新颖，而且在目标设计约束和新兴蛋白质特性指标上都显示出改进特征。

Conclusion: 该方法为蛋白质语言模型提供了一种简单通用的快速监督微调方案，能够生成更稳定、功能性更强的酶，同时扩展了对蛋白质序列空间的探索，超越了天然变体。
...
### [37] [Representation Invariance and Allocation: When Subgroup Balance Matters](https://arxiv.org/abs/2512.09496)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法假设平衡子群表示能优化模型性能，但近期实证结果与此矛盾：有时不平衡数据分布反而改善子群性能，有时即使训练数据中完全缺失某个子群，其性能也不受影响。这促使研究者系统探究子群分配对模型性能的影响机制。

Method: 1. 在四个视觉和语言模型上进行系统研究，改变训练数据组成以表征子群性能对数据平衡的敏感性；2. 提出潜在分离假说：部分微调模型对子群表示的依赖程度由预训练模型潜在空间中子群的分离程度决定；3. 形式化该假说，提供理论分析，并进行实证验证；4. 展示在基础模型微调中的实际应用。

Result: 研究验证了潜在分离假说，表明子群性能对数据平衡的敏感性确实取决于预训练模型潜在空间中子群的分离程度。这为数据收集和平衡决策提供了量化分析框架。

Conclusion: 传统的数据平衡假设过于简化，实际效果取决于预训练模型的特征表示。通过量化分析潜在子群分离程度，可以更智能地指导数据收集和平衡策略，优化模型在不同人口群体间的泛化性能。
...
### [38] [Improved Physics-Driven Neural Network to Solve Inverse Scattering Problems](https://arxiv.org/abs/2512.09333)
*Yutong Du,Zicheng Liu,Bo Wu,Jingwei Kou,Hang Li,Changyou Li,Yali Zong,Bo Qi*

Main category: cs.LG

TL;DR: 提出改进的物理驱动神经网络框架，用于电磁逆散射问题求解，通过新型激活函数、动态散射子区域识别和迁移学习，实现高精度、鲁棒且高效的重建。
...
Method: 1. 提出高斯局部化振荡抑制窗口激活函数稳定收敛并实现轻量网络架构；2. 开发动态散射子区域识别策略自适应细化计算域；3. 结合迁移学习扩展求解器到实际场景。

Result: 数值模拟和实验结果表明，该方法在重建精度、鲁棒性和效率方面优于现有最先进方法，实现了物理可解释性与实时推理能力的结合。

Conclusion: IPDNN框架成功解决了电磁逆散射问题的多个挑战，为实际应用提供了准确、高效且鲁棒的解决方案，展示了物理驱动神经网络在科学计算中的潜力。
...
### [39] [Stanford Sleep Bench: Evaluating Polysomnography Pre-training Methods for Sleep Foundation Models](https://arxiv.org/abs/2512.09591)
...
<details>
  <summary>Details</summary>
Motivation: 多导睡眠图产生大量多模态临床数据，为自监督表示学习提供了机会，但睡眠基础模型的发展受到两个关键限制：缺乏共享数据集和基准，以及缺乏对自监督学习方法在睡眠相关任务中的系统评估。

Method: 引入斯坦福睡眠基准，这是一个包含17,467个记录（总计超过163,000小时）的大型多导睡眠图数据集，包含13个临床疾病预测任务以及睡眠分期、呼吸暂停诊断和年龄估计等经典任务。系统评估了自监督预训练方法在这些任务上的表现。

Result: 多个预训练方法在睡眠分期、呼吸暂停诊断和年龄估计任务上表现相当。然而，在死亡率和疾病预测任务中，对比学习显著优于其他方法，并且在预训练期间收敛更快。

Conclusion: 斯坦福睡眠基准填补了睡眠基础模型研究的关键空白，为睡眠研究提供了共享数据集和系统评估框架。对比学习在临床预测任务中表现出色，将发布数据集、预训练模型权重、训练管道和评估代码以促进可重复性和研究进展。
...
### [40] [Drawback of Enforcing Equivariance and its Compensation via the Lens of Expressive Power](https://arxiv.org/abs/2512.09673)
*Yuzhu Chen,Tian Qin,Xinmei Tian,Fengxiang He,Dacheng Tao*

Main category: cs.LG

TL;DR: 本文研究了等变神经网络在2层ReLU网络中的表达能力，发现等变约束会限制表达能力，但可通过扩大模型规模来补偿，且等变网络仍具有较低复杂度，泛化能力更好。
...
Conclusion: 等变神经网络虽然表达能力受限，但通过适当扩大模型规模可以补偿，且由于其假设空间复杂度更低，具有更好的泛化性能。
...
### [41] [A Granular Framework for Construction Material Price Forecasting: Econometric and Machine-Learning Approaches](https://arxiv.org/abs/2512.09360)
...
<details>
  <summary>Details</summary>
Motivation: 建筑材料价格的持续波动给成本估算、预算编制和项目交付带来了显著风险，迫切需要更精细和可扩展的预测方法。

Method: 研究开发了一个预测框架，以CSI MasterFormat为目标数据结构，在六位数章节级别进行预测。框架整合了原材料价格、商品指数和宏观经济指标等解释变量。评估了四种时间序列模型：LSTM、ARIMA、VECM和Chronos-Bolt，分别在仅使用CSI数据的基准配置和包含解释变量的扩展版本中进行比较。

Result: 结果表明，整合解释变量显著提高了所有模型的预测性能。LSTM模型表现最佳，RMSE值低至1.390，MAPE值为0.957，比传统统计时间序列模型ARIMA提升了高达59%。在多个CSI部门的验证证实了框架的可扩展性。

Conclusion: 该研究提供了一个稳健的方法论，使业主和承包商能够改进预算实践，在确定性水平上实现更可靠的成本估算。框架的细粒度预测能力有助于应对建筑材料价格波动带来的风险。
...
### [42] [Circuits, Features, and Heuristics in Molecular Transformers](https://arxiv.org/abs/2512.09757)
*Kristof Varadi,Mark Marosi,Peter Antal*

Main category: cs.LG

TL;DR: 该论文对训练在药物类小分子上的自回归Transformer进行机制分析，揭示了模型在多个抽象层次上捕捉分子表示规则的计算结构，发现了与低层语法解析和更高层化学有效性约束一致的计算模式。


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer模型能够生成有效且多样化的化学结构，但人们对这些模型如何捕捉分子表示规则的机制知之甚少。本研究旨在揭示自回归Transformer在化学分子生成中的计算机制。

Method: 使用稀疏自编码器（SAEs）提取与化学相关激活模式相关的特征字典，对训练在药物类小分子上的自回归Transformer进行机制分析，识别不同抽象层次的计算模式。

Result: 发现了与低层语法解析和更高层化学有效性约束一致的计算模式，提取了与化学相关激活模式相关的特征字典，并在下游任务中验证了这些发现，表明机制洞察可以转化为各种实际场景中的预测性能。

Conclusion: 通过对化学Transformer的机制分析，揭示了模型捕捉分子表示规则的多层次计算结构，为理解深度学习模型在化学领域的内部工作机制提供了重要见解，并展示了机制分析对实际应用的价值。
...
### [43] [KGOT: Unified Knowledge Graph and Optimal Transport Pseudo-Labeling for Molecule-Protein Interaction Prediction](https://arxiv.org/abs/2512.09365)
*Jiayu Qin,Zhengquan Luo,Guy Tadmor,Changyou Chen,David Zeevi,Zhiqiang Xu*

Main category: cs.LG

TL;DR: 提出了一种基于最优传输的分子-蛋白质相互作用预测框架，通过整合多种生物数据源和生成高质量伪标签来解决数据稀缺问题


<details>
  <summary>Details</summary>
Motivation: 现有分子-蛋白质相互作用预测模型面临两个主要挑战：1) 标记数据稀缺，可用数据集仅捕获了生物相关相互作用的一小部分；2) 大多数方法仅依赖分子和蛋白质特征，忽略了更广泛的生物背景信息

Method: 首先整合多种生物数据集（分子、蛋白质、基因和通路水平相互作用），然后开发基于最优传输的方法为未标记的分子-蛋白质对生成高质量伪标签，利用已知相互作用的底层分布来指导标签分配

Result: 在多个MPI数据集（包括虚拟筛选任务和蛋白质检索任务）上评估，在预测准确性和零样本能力方面相比最先进方法有显著提升

Conclusion: 该方法不仅改进了MPI预测，还为利用多样化生物数据源解决传统上受限于单模态或双模态学习的问题提供了新范式，为计算生物学和药物发现的未来发展铺平了道路
...
### [44] [Provably Learning from Modern Language Models via Low Logit Rank](https://arxiv.org/abs/2512.09892)
*Noah Golowich,Allen Liu,Abhishek Shetty*

Main category: cs.LG

TL;DR: 论文提出了一种基于低对数秩假设的语言模型学习算法，该算法能够从查询中高效学习近似低对数秩模型，为现代语言模型提供了首个端到端学习保证。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型虽然复杂，但经验观察显示它们都具有近似低对数秩的特性。本文旨在探索如何利用这种结构特性来获得可证明的学习保证，特别是考虑到低对数秩模型可以编码难以学习的分布（如噪声奇偶性）。

Method: 采用查询学习模型，使用对数查询来反映常见API的访问模式。提出了一种高效算法，可以从查询中学习任何近似低对数秩模型。

Result: 主要结果是开发了一个高效算法，能够从查询中学习任何近似低对数秩模型。该算法为生成模型提供了首个端到端学习保证，且结构假设与现代语言模型的经验观察行为紧密对应。

Conclusion: 本文通过利用语言模型的低对数秩特性，提出了一种具有可证明学习保证的高效学习算法，为现代语言模型的学习理论提供了重要基础。
...
### [45] [CFLight: Enhancing Safety with Traffic Signal Control through Counterfactual Learning](https://arxiv.org/abs/2512.09368)
...
<details>
  <summary>Details</summary>
Motivation: 交通信号控制中的强化学习方法通常过于关注效率而忽视安全性，且缺乏可解释性，需要平衡安全与效率并提高方法透明度

Method: 提出基于反事实学习的CFLight框架，构建结构因果模型预测不同行动的结果，通过反事实模块与额外"X"模块集成，实现近零碰撞控制策略

Result: 在真实世界和合成数据集上的实验表明，CFLight相比传统强化学习方法和近期安全强化学习模型，能减少碰撞并提升整体交通性能

Conclusion: CFLight为强化学习方法提供了一个通用且安全的框架，能够在交通信号控制中有效处理安全事件，并可扩展到其他领域应用
...
### [46] [STACHE: Local Black-Box Explanations for Reinforcement Learning Policies](https://arxiv.org/abs/2512.09909)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习智能体在稀疏奖励或安全关键环境中经常表现出意外行为，需要可靠的调试和验证工具来理解智能体的决策过程。

Method: 提出STACHE框架，生成包含鲁棒性区域（智能体决策不变的连通邻域）和最小反事实（改变决策所需的最小状态扰动）的复合解释。利用因子化状态空间结构，引入精确的基于搜索的算法，避免代理模型的保真度差距。

Result: 在Gymnasium环境中的实证验证表明，该框架不仅能解释策略动作，还能有效捕捉训练过程中策略逻辑的演变——从不稳定行为到优化稳健策略，提供对智能体敏感性和决策边界的可操作见解。

Conclusion: STACHE为强化学习智能体的行为解释提供了全面框架，通过鲁棒性区域和最小反事实的互补分析，帮助理解智能体决策机制和训练动态，对调试和验证具有重要意义。
...
### [47] [Are Hypervectors Enough? Single-Call LLM Reasoning over Knowledge Graphs](https://arxiv.org/abs/2512.09369)
*Yezi Liu,William Youngwoo Chung,Hanning Chen,Calvin Yeung,Mohsen Imani*

Main category: cs.LG

TL;DR: PathHD：基于超维度计算的轻量级知识图谱推理框架，用HDC替代神经路径评分，每个查询只需一次LLM调用，在保持性能的同时大幅降低延迟和GPU成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的LLM推理方法依赖重型神经编码器进行路径嵌入评分，或需要重复LLM调用来排序候选，导致高延迟、高GPU成本和不透明的决策，阻碍了可扩展的忠实部署。

Method: 提出PathHD框架：1）使用超维度计算（HDC）替代神经路径评分；2）将关系路径编码为块对角GHRR超向量；3）通过块级余弦相似度和Top-K剪枝对候选进行排序；4）执行一次性LLM裁决生成最终答案并引用支持路径。

Result: 在WebQSP、CWQ和GrailQA数据集上：1）每个查询只需一次LLM调用即可达到或超过强神经基线的Hits@1性能；2）端到端延迟降低40-60%，GPU内存减少3-5倍；3）提供忠实、基于路径的可解释性，改善错误诊断和可控性。

Conclusion: 精心设计的HDC表示为高效的KG-LLM推理提供了实用基础，在准确性、效率和可解释性之间提供了有利的权衡，表明轻量级超维度计算是替代重型神经编码器的可行方案。
...
### [48] [FALCON: Few-step Accurate Likelihoods for Continuous Flows](https://arxiv.org/abs/2512.09914)
...
<details>
  <summary>Details</summary>
Motivation: 当前Boltzmann生成器主要使用连续归一化流，但其似然计算成本极高，每个样本需要数千次函数评估，严重限制了应用。

Method: 提出FALCON方法，通过引入混合训练目标来鼓励可逆性，实现少量步骤的精确似然采样。

Result: FALCON在分子Boltzmann采样方面优于最先进的归一化流模型，比同等性能的CNF模型快两个数量级。

Conclusion: FALCON方法解决了连续流模型似然计算成本高的问题，为热力学平衡态分子采样提供了高效解决方案。
...
TL;DR: 该论文扩展了科学机器学习的学习理论，证明双纤维化变换这类几何积分算子可以从极少训练样本中高效学习，误差衰减速度超过样本数量的任何固定幂次，避免了维度灾难。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习理论主要针对简单几何的椭圆算子，但科学机器学习常涉及奇异性传播问题（如波动、对流、流体动力学）。需要扩展理论以涵盖双纤维化变换这类几何积分算子。

Method: 扩展学习理论以包含双纤维化变换（包括广义Radon变换和测地线射线变换）。提出一种基于水平集方法的交叉注意力架构，显式编码这些变换的几何结构。

Result: 证明双纤维化变换类算子不存在维度灾难：误差以超代数速度衰减（快于训练样本数量倒数的任何固定幂次）。提出的架构具有普适性、稳定性，并能从极少训练样本中学习双纤维化变换。

Conclusion: 该研究扩展了科学机器学习算子学习的理论框架，为涉及奇异性传播的问题提供了理论基础，证明了特定几何积分算子的高效可学习性。
...
### [50] [Federated Distillation Assisted Vehicle Edge Caching Scheme Based on Lightweight DDPM](https://arxiv.org/abs/2512.09378)
*Xun Li,Qiong Wu,Pingyi Fan,Kezhi Wang,Wen Chen,Khaled B. Letaief*

Main category: cs.LG

TL;DR: 提出基于轻量级去噪扩散概率模型的联邦蒸馏辅助车辆边缘缓存方案，解决传统联邦学习的通信开销大和车辆移动导致的训练失败问题


<details>
  <summary>Details</summary>
Motivation: 车辆边缘缓存能降低车辆用户访问内容的延迟，但需要准确预测用户兴趣内容而不暴露隐私。传统联邦学习虽然能保护隐私，但存在通信开销大和车辆移动导致训练失败的问题

Method: 提出基于轻量级去噪扩散概率模型的联邦蒸馏辅助车辆边缘缓存方案，通过联邦蒸馏减少模型传输频率，降低通信开销，并适应车辆移动性

Result: 仿真结果表明，所提方案对车辆速度变化具有良好的鲁棒性，显著降低了通信开销并提高了缓存命中率

Conclusion: 基于LDPM的联邦蒸馏辅助车辆边缘缓存方案能有效解决传统联邦学习在车辆边缘缓存中的通信开销和移动性问题，提高系统性能
...
### [51] [Black-Box Behavioral Distillation Breaks Safety Alignment in Medical LLMs](https://arxiv.org/abs/2512.09403)
*Sohely Jahan,Ruimin Sun*

Main category: cs.LG

TL;DR: 本文提出一种针对医疗大语言模型的黑盒蒸馏攻击方法，仅通过输出访问就能复制安全对齐模型的专业推理能力，同时剥离其安全机制，揭示功能-伦理差距的威胁。


<details>
  <summary>Details</summary>
Motivation: 随着医疗大语言模型在临床工作流中的集成度提高，对其对齐鲁棒性和安全性的担忧日益增加。先前的研究主要关注分类模型或记忆泄漏，而安全对齐的生成式医疗大语言模型的脆弱性尚未得到充分探索。

Method: 提出黑盒蒸馏攻击：1) 向Meditron-7B发出48,000条指令查询，收集25,000个良性指令-响应对；2) 在零对齐监督设置下，通过参数高效的LoRA方法微调LLaMA3 8B代理模型；3) 开发动态对抗评估框架，结合生成查询有害提示生成、验证器过滤、类别化失败分析和自适应随机搜索越狱攻击。

Result: 仅花费12美元，代理模型在良性输入上保持高保真度，同时对86%的对抗性提示产生不安全完成，远超Meditron-7B（66%）和未调优的基础模型（46%）。这揭示了显著的功能-伦理差距：任务效用转移而安全对齐崩溃。

Conclusion: 仅基于良性数据的黑盒蒸馏暴露了一个实际且未被充分认识的威胁：攻击者可以廉价复制医疗大语言模型的能力，同时剥离安全机制，强调了需要提取感知的安全监控。提出了分层防御系统作为黑盒部署中实时对齐漂移的原型检测器。
...
### [52] [Cauchy-Schwarz Fairness Regularizer](https://arxiv.org/abs/2512.09467)
*Yezi Liu,Hanning Chen,Wenjun Huang,Yang Ni,Mohsen Imani*

Main category: cs.LG

TL;DR: 本文提出了一种基于柯西-施瓦茨散度的公平性正则化器，用于解决机器学习中的群体公平性问题，相比现有方法具有更紧的泛化界、对尺度差异的鲁棒性，并能处理任意预测分布。


<details>
  <summary>Details</summary>
Motivation: 现有公平性正则化器基于不同的距离度量和设计选择，导致其行为难以推理且在不同任务上表现不一致。需要探究什么特性构成一个好的公平性正则化器，并设计具有理想特性的新方法。

Method: 将现有方法分为三类：跨敏感组匹配预测统计量、对齐潜在表示、直接最小化预测与敏感属性之间的依赖性。提出基于柯西-施瓦茨散度的公平性正则化器，惩罚敏感组条件预测分布之间的经验CS散度。该方法采用分布无关的核基估计器，可自然扩展到多个敏感属性。

Result: 在四个表格基准数据集和一个图像数据集上的广泛实验表明，提出的CS正则化器在保持竞争性准确度的同时，一致改善了人口统计均等和机会均等指标，并且相比先前正则化器在超参数设置下实现了更稳定的效用-公平性权衡。

Conclusion: 柯西-施瓦茨散度作为公平性正则化器具有理论优势，包括更紧的泛化界、对尺度差异的鲁棒性，以及处理任意预测分布的能力。提出的CS正则化器在实际应用中表现出色，为机器学习公平性提供了更可靠和一致的解决方案。
...
### [53] [Contextual Dynamic Pricing with Heterogeneous Buyers](https://arxiv.org/abs/2512.09513)
*Thodoris Lykouris,Sloan Nietert,Princewill Okoroafor,Chara Podimata,Julian Zimmert*

Main category: cs.LG

TL;DR: 论文研究了具有异质买家群体的上下文动态定价问题，提出了基于乐观后验采样的算法，实现了$\widetilde{O}(K_{\star}\sqrt{dT})$的遗憾上界，并在非上下文情况下设计了方差感知的缩放算法。


<details>
  <summary>Details</summary>
Motivation: 现有动态定价研究通常假设买家同质，但现实中买家类型存在异质性。本文首次研究具有异质买家群体的上下文动态定价问题，其中买家估值类型来自未知的有限支撑分布。

Method: 提出了基于乐观后验采样的上下文定价算法，利用买家类型的有限支撑特性。对于非上下文情况，设计了方差感知的缩放算法，能够更好地处理买家类型的异质性。

Result: 上下文定价算法实现了$\widetilde{O}(K_{\star}\sqrt{dT})$的遗憾上界，该上界在$d$和$T$维度上达到紧致（忽略对数项）。非上下文情况下的方差感知缩放算法在$K_{\star}$依赖上达到最优。

Conclusion: 本文为异质买家群体的动态定价问题提供了理论保证，证明了乐观后验采样方法的有效性，并在非上下文情况下通过方差感知技术进一步优化了算法性能。
...
### [54] [QuanvNeXt: An end-to-end quanvolutional neural network for EEG-based detection of major depressive disorder](https://arxiv.org/abs/2512.09517)
*Nabil Anan Orka,Ehtashamul Haque,Maftahul Jannat,Md Abdul Awal,Mohammad Ali Moni*

Main category: cs.LG

TL;DR: QuanvNeXt是一个用于基于EEG的抑郁症诊断的端到端全量子卷积模型，通过创新的Cross Residual块减少特征同质性并增强跨特征关系，在两个开源数据集上取得了93.1%的平均准确率和97.2%的平均AUC-ROC，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 开发一个高效可靠的基于EEG的抑郁症诊断模型，解决现有方法中可能存在的特征同质性问题，同时保持参数效率，为临床抑郁症诊断提供更准确的工具。

Method: 提出了QuanvNeXt模型，这是一个端到端的全量子卷积模型，核心创新是Cross Residual块，该块能够减少特征同质性并增强跨特征关系。模型在两个开源EEG数据集上进行评估，进行了不确定性分析（高斯噪声水平）和后验可解释AI分析。

Result: 在两个数据集上平均准确率达到93.1%，平均AUC-ROC为97.2%，优于InceptionTime等基线方法（91.7%准确率，95.9% AUC-ROC）。不确定性分析显示即使在最高扰动（ε=0.1）下，ECE分数保持较低（0.0436）到中等（0.1159）。可解释性分析确认模型能有效识别和学习区分健康对照与重度抑郁症的频谱时间模式。

Conclusion: QuanvNeXt为基于EEG的抑郁症诊断建立了一个高效可靠的方法，通过创新的Cross Residual块在保持参数效率的同时提升了性能，并在不确定性和可解释性方面表现出色。
...
### [55] [Latent-Autoregressive GP-VAE Language Model](https://arxiv.org/abs/2512.09535)
*Yves Ruffenach*
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型中时间结构是否可以通过潜在空间的概率几何来支持，而不是完全依赖显式的神经操作，从而将序列动态与并行生成分离。

Method: 提出完全潜在自回归方案，包含因果高斯过程先验、结构化摊销后验，以及基于正则化ELBO的训练协议，将序列动态转移到连续潜在空间，同时使用非自回归解码器进行并行语言生成。

Result: 在概念验证框架下的实验表明，模型能够稳定训练，序列和并行采样变体表现出一致行为，表明语言模型的部分时间结构可以通过潜在空间的概率几何来支持。

Conclusion: 语言模型的时间结构可以部分地由潜在空间的概率几何支持，而不是完全依赖显式神经操作，这为序列建模提供了新的视角。
...
### [56] [Semantic-Aware Cooperative Communication and Computation Framework in Vehicular Networks](https://arxiv.org/abs/2512.09621)
*Jingbo Zhang,Maoxin Ji,Qiong Wu,Pingyi Fan,Kezhi Wang,Wen Chen*

Main category: cs.LG

TL;DR: 本文提出了一种用于车联网的三方协作语义通信框架，通过V2I和V2V通信实现语义任务卸载，使用MAPPO-PDN算法优化语义符号数量，线性规划解决卸载比例，在高速公路场景中性能优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 车联网中结合语义通信和车辆边缘计算可以提升边缘任务处理效率，但现有方法在高速公路场景下的任务延迟和语义符号优化方面存在挑战，需要更有效的协同卸载方案。

Method: 提出三方协作语义通信框架，将混合整数非线性规划问题分解为两个子问题：1）使用基于参数分布噪声的多智能体近端策略优化算法优化语义符号数量；2）使用线性规划解决卸载比例问题。

Result: 仿真结果表明，该方案在任务延迟和语义符号数量优化方面的性能优于其他对比算法。

Conclusion: TCSC框架为车联网提供了一种高效的语义任务卸载方案，通过V2I和V2V协同通信以及创新的优化算法，显著提升了高速公路场景下的边缘计算性能。
...
### [57] [Membership and Dataset Inference Attacks on Large Audio Generative Models](https://arxiv.org/abs/2512.09654)
*Jakub Proboszcz,Paweł Kochanski,Karol Korszun,Donato Crisostomi,Giorgio Strano,Emanuele Rodolà,Kamil Deja,Jan Dubinski*

Main category: cs.LG

TL;DR: 该研究探讨了在开源生成音频模型中通过成员推断攻击验证艺术家作品是否被用于训练的可能性，发现单个样本的成员推断效果有限，但通过数据集推断聚合多个样本证据可有效检测艺术家作品是否被用于模型训练。


<details>
  <summary>Details</summary>
Motivation: 随着生成音频模型（基于扩散和自回归架构）在质量和表现力方面的快速发展，引发了版权担忧。这些模型通常在大量艺术和商业作品上进行训练，因此需要可靠的方法来验证艺术家的材料是否被包含在训练集中，为版权持有者提供保护内容的手段。

Method: 研究通过成员推断攻击（MIA）在开源生成音频模型上进行实验，尝试确定特定音频样本是否属于训练集。同时借鉴文本和视觉领域的工作，专注于数据集推断（DI），该方法聚合多个样本的成员证据。

Result: 实证结果显示，对于在大型多样化数据集上训练的模型，单个样本的成员推断信号较弱，效果有限。然而，数据集推断在音频领域是成功的，能够通过聚合多个样本的证据来更有效地评估艺术家的作品是否对模型训练有贡献。

Conclusion: 数据集推断为大型音频生成模型时代的版权保护和数据集问责制提供了一个有前景的方向，为艺术家和媒体所有者提供了一种实用的机制来验证其作品是否被用于模型训练。
...
### [58] [A data-driven approach to linking design features with manufacturing process data for sustainable product development](https://arxiv.org/abs/2512.09690)
...
<details>
  <summary>Details</summary>
Motivation: 工业物联网技术在制造业的广泛应用实现了制造过程数据的自动化实时收集，为数据驱动的产品开发提供了新机遇。然而，当前的数据驱动方法通常局限于特定领域（如设计或制造），缺乏对设计特征与制造过程数据整合的探索。由于设计决策显著影响制造结果（如错误率、能耗和处理时间），这种整合的缺失限制了数据驱动产品设计改进的潜力。

Method: 本文提出了一种数据驱动方法来映射和分析设计特征与制造过程数据之间的关系。开发了全面的系统架构以确保连续的数据收集和整合。基于设计特征与制造过程数据的关联，开发了机器学习模型，能够自动提供设计改进建议。通过将制造过程数据与可持续性指标整合，该方法为可持续产品开发开辟了新可能性。

Result: 该方法建立了设计特征与制造过程数据之间的系统映射关系，开发了能够自动提供设计改进建议的机器学习模型。通过整合可持续性指标，为可持续产品开发提供了新的数据驱动框架。

Conclusion: 本文提出的数据驱动方法成功整合了设计特征与制造过程数据，通过机器学习模型实现了自动化设计改进。该方法不仅提高了产品设计的效率和质量，还为可持续产品开发提供了新的技术路径，具有重要的工业应用价值。
...
### [59] [Training One Model to Master Cross-Level Agentic Actions via Reinforcement Learning](https://arxiv.org/abs/2512.09706)
*Kaichen He,Zihao Wang,Muyao Li,Anji Liu,Yitao Liang*

Main category: cs.LG

TL;DR: CrossAgent是一个统一的智能体模型，能够掌握异构动作空间并在轨迹的每个步骤中自主选择最有效的交互接口，通过自适应动作切换在开放世界任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体通常局限于静态、预定义的动作空间（如API、GUI事件或机器人命令），这种刚性限制了它们在动态环境中的适应性，因为最佳交互粒度会因上下文而变化。

Method: 提出了CrossAgent统一智能体模型，采用综合训练流程，结合冷启动监督微调和多轮组相对策略优化（GRPO）算法，使智能体能够学习自适应动作切换，无需人工指定规则。

Result: 在开放世界Minecraft环境中的800多个任务上进行广泛实验，CrossAgent实现了最先进的性能，通过动态利用多样化动作空间的优势，显著优于固定动作基线，在长时程推理中表现出优越的泛化能力和效率。

Conclusion: CrossAgent通过掌握异构动作空间和自适应选择最有效接口，解决了现有智能体在动态环境中适应性的限制，为智能体AI从工程化复杂工作流向原生模型后训练范式转变提供了有效解决方案。
...
### [60] [Mixture of Lookup Key-Value Experts](https://arxiv.org/abs/2512.09723)
*Zongcheng Wang*

Main category: cs.LG

TL;DR: 本文提出MoLKV模型，通过上下文感知的专家选择机制改进MoLE架构，在资源受限设备上实现更好的性能。


<details>
  <summary>Details</summary>
Motivation: MoLE架构虽然适合终端设备推理，但其基于输入ID的上下文无关专家选择机制可能限制模型性能，需要更智能的专家激活策略。

Method: 提出MoLKV模型，将每个专家构建为键值对，通过输入派生的查询与当前序列缓存的键值专家交互，生成上下文感知的专家输出。

Result: 实验结果表明，MoLKV在小规模评估中实现了显著更低的验证损失，证明上下文感知机制的有效性。

Conclusion: MoLKV通过引入上下文感知的专家选择机制，成功解决了MoLE的性能限制，为资源受限设备上的LLM推理提供了改进方案。
...
### [61] [Physics-Aware Heterogeneous GNN Architecture for Real-Time BESS Optimization in Unbalanced Distribution Systems](https://arxiv.org/abs/2512.09780)
*Aoxiang Ma,Salah Ghamizi,Jun Cao,Pedro Rodriguez*

Main category: cs.LG

TL;DR: 该论文提出了一种基于异构图神经网络的三相不平衡配电网电池储能系统调度方法，通过嵌入详细的三相电网信息和使用物理信息损失函数，实现了高精度的网络状态预测和近乎零约束违反的可靠调度。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在三相不平衡配电网中缺乏明确的三相表示，难以准确建模相特定动态和强制执行运行约束，导致调度方案不可行。需要开发能够准确建模三相动态并确保约束合规的方法。

Method: 将详细的三相电网信息（包括相电压、不平衡负载和BESS状态）嵌入到异构图节点中，采用多种GNN架构（GCN、GAT、GraphSAGE、GPS）联合预测网络状态变量。使用物理信息损失函数，通过软惩罚在训练中纳入关键的电池约束（SoC和C-rate限制）。

Result: 在CIGRE 18节点配电系统上的实验验证显示，该方法实现了低预测误差：总线电压MSE分别为GCN 6.92e-07、GAT 1.21e-06、GPS 3.29e-05、SAGE 9.04e-07。更重要的是，物理信息方法确保了近乎零的SoC和C-rate约束违反，证明了其在可靠、约束合规调度中的有效性。

Conclusion: 通过将详细的三相电网信息嵌入异构图节点并结合物理信息损失函数，该方法能够准确预测网络状态并确保电池约束合规，为三相不平衡配电网中的电池储能系统提供了可靠、可行的调度解决方案。
...
### [62] [Predicting Polymer Solubility in Solvents Using SMILES Strings](https://arxiv.org/abs/2512.09784)
*Andrew Reinhard*

Main category: cs.LG

TL;DR: 该研究开发了一个深度学习框架，直接从聚合物和溶剂的SMILES表示预测聚合物溶解度（wt%），在8049个聚合物-溶剂对数据集上训练，并在实验数据上验证了良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 理解和预测聚合物在不同溶剂中的溶解度对于从回收到药物制剂等应用至关重要。传统方法通常耗时且成本高，需要开发更高效、可扩展的预测方法。

Method: 构建了包含8049个聚合物-溶剂对的数据集（来自分子动力学模拟），将分子描述符和指纹组合成2394个特征表示，使用具有六个隐藏层的全连接神经网络，采用Adam优化器和均方误差损失进行训练。

Result: 模型在预测溶解度方面表现出色，预测值与实际值高度一致。在材料基因组项目的实验数据上测试时，模型在25个未见过的聚合物-溶剂组合上保持了高精度，证明了良好的泛化能力。

Conclusion: 基于SMILES的机器学习模型可用于可扩展的溶解度预测和高通量溶剂筛选，支持绿色化学、聚合物加工和材料设计等应用，展示了深度学习在聚合物溶解度预测中的可行性。
...
### [63] [TinyDéjàVu: Smaller Memory Footprint & Faster Inference on Sensor Data Streams with Always-On Microcontrollers](https://arxiv.org/abs/2512.09786)
*Zhaolan Huang,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: TinyDéjàVu框架通过优化数据流和消除冗余计算，显著降低微控制器上传感器时间序列推理的RAM占用


<details>
  <summary>Details</summary>
Motivation: 随着始终在线传感器需要运行各种小型神经网络进行持续推理，而微控制器内存预算有限（如128kB RAM），优化神经网络层间的数据流变得至关重要

Method: 开发了TinyDéjàVu框架和新型算法，通过优化数据流和消除重叠滑动窗口输入的冗余计算来减少RAM占用

Result: TinyDéjàVu可以节省超过60%的RAM使用，并消除高达90%的重叠滑动窗口输入的冗余计算

Conclusion: TinyDéjàVu框架通过开源实现和硬件基准测试，为微控制器上的传感器时间序列推理提供了有效的RAM优化解决方案
...
### [64] [Knowledge Diversion for Efficient Morphology Control and Policy Transfer](https://arxiv.org/abs/2512.09796)
...
Method: 通过SVD将随机初始化的Transformer权重分解为因子单元，使用动态软门控根据任务和形态嵌入调制这些单元，分离为共享的"learngenes"和形态/任务特定的"tailors"，实现知识解耦。

Result: DivMorph达到最先进性能，在跨任务迁移中比直接微调提升3倍样本效率，单智能体部署时模型大小减少17倍。

Conclusion: DivMorph通过模块化设计和知识解耦，实现了可扩展、高效的策略部署，支持向新任务的有效策略迁移。
...
### [65] [Ariel-ML: Computing Parallelization with Embedded Rust for Neural Networks on Heterogeneous Multi-core Microcontrollers](https://arxiv.org/abs/2512.09800)
*Zhaolan Huang,Kaspar Schleiser,Gyungmin Myung,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: Ariel-ML是一个基于Rust的嵌入式软件平台，专门为多核微控制器上的TinyML推理计算提供自动化并行化支持，填补了现有工具链的空白。


<details>
  <summary>Details</summary>
Motivation: 随着低功耗微控制器从单核向多核架构演进，以及Rust在嵌入式领域的兴起，现有工具链缺乏能够自动化利用多核能力进行TinyML模型推理的Rust嵌入式平台。

Method: 设计并实现了Ariel-ML工具包，结合通用TinyML流程和嵌入式Rust软件平台，支持多种32位微控制器家族（Arm Cortex-M、RISC-V、ESP-32），并开源了完整代码。

Result: Ariel-ML在推理延迟方面优于现有技术，与使用嵌入式C/C++的现有工具包相比，内存占用相当，为TinyML实践者和资源受限的嵌入式Rust开发者提供了有用基础。

Conclusion: Ariel-ML填补了多核MCU上TinyML推理自动化并行化的空白，提供了性能优越且内存效率高的解决方案，推动了边缘AI在嵌入式系统中的应用。
...
TL;DR: 该研究提出了一种在kNN和ε邻域图构建阶段就引入公平性约束的新方法，通过在邻域选择中确保敏感特征的均衡表示，从而在预处理阶段实现公平的谱聚类。


<details>
  <summary>Details</summary>
Motivation: 传统图聚类方法（如谱聚类）在构建图时存在偏见，可能对某些群体代表性不足。现有的公平性研究主要集中在聚类算法本身，而忽略了图构建阶段的公平性问题，这会导致边缘选择中的不公平影响传播到聚类结果中。

Method: 提出了两种公平图构建方法：公平kNN图和公平ε邻域图。通过在邻域选择步骤中主动实施人口统计平等约束，确保每个节点的邻域中敏感群体都有均衡的比例表示，同时保持几何一致性。

Result: 在三个合成数据集、七个真实世界表格数据集和三个真实世界图像数据集上的实验证明，所提出的公平图构建方法在公平谱聚类任务中超越了现有基线方法。

Conclusion: 图构建阶段的拓扑公平性对于实现公平聚类结果至关重要。通过在预处理阶段确保邻域选择的公平性，可以在不修改聚类算法本身的情况下获得更公平的谱聚类结果，填补了公平无监督学习中的重要空白。
...
### [67] [Predicting the Containment Time of California Wildfires Using Machine Learning](https://arxiv.org/abs/2512.09835)
*Shashank Bhardwaj*
...
Method: 整合加州林业和消防部门FRAP的三个公开数据集，将野火持续时间预测作为回归任务。比较了基线集成回归器、随机森林、XGBoost和LSTM神经网络模型。

Result: XGBoost模型略优于随机森林，可能因其对静态特征处理更佳。LSTM模型表现最差，因为数据集缺乏时序特征。研究显示根据特征可用性可选择最合适模型。

Conclusion: 野火管理者可根据特征可用性选择合适模型准确预测控制持续时间，有效分配资源。回归方法比分类方法提供更详细精确的预测。
...
### [68] [Conformal Bandits: Bringing statistical validity and reward efficiency to the small-gap regime](https://arxiv.org/abs/2512.09850)
*Simone Cuonzo,Nina Deliu*

Main category: cs.LG

TL;DR: 将Conformal Prediction（CP）整合到bandit问题中，通过CP为决策提供有限时间预测覆盖保证，在small-gap场景中优于传统策略，并在投资组合分配应用中结合隐马尔可夫模型提升风险调整收益。


<details>
  <summary>Details</summary>
Motivation: 传统bandit策略（如Thompson Sampling和UCB）通常依赖分布假设或渐近保证，且主要关注遗憾最小化，忽视了统计特性。本文旨在填补这一空白，将CP的统计保证与bandit策略的遗憾最小化潜力相结合。

Method: 提出Conformal Bandits框架，将Conformal Prediction整合到bandit问题中。在投资组合分配应用中，进一步结合隐马尔可夫模型来捕捉金融市场的状态转换行为，优化探索-利用权衡。

Result: 在small-gap场景（如投资组合分配）中，Conformal Bandits在遗憾方面优于传统UCB策略，同时实现名义覆盖保证。结合隐马尔可夫模型后，在保持覆盖保证的同时，实现了更高的风险调整收益效率。

Conclusion: Conformal Bandits框架成功地将CP的统计保证与bandit策略的遗憾最小化潜力相结合，在small-gap场景中表现出色，特别是在金融应用领域，为决策提供了可靠的统计保证。
...
### [69] [HPM-KD: Hierarchical Progressive Multi-Teacher Framework for Knowledge Distillation and Efficient Model Compression](https://arxiv.org/abs/2512.09886)
*Gustavo Coelho Haase,Paulo Henrique Dourado da Silva*

Main category: cs.LG

TL;DR: HPM-KD是一个知识蒸馏框架，通过六个协同组件解决传统KD的四大限制：自动超参数调优、渐进蒸馏链、注意力加权多教师集成、元学习温度调度、并行处理管道和共享优化内存，实现10-15倍压缩且保持85%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏技术面临四个关键限制：1) 对超参数敏感需要大量手动调优；2) 从大教师模型到小学生模型存在容量差距；3) 多教师场景下协调不优；4) 计算资源使用效率低。需要一种更自动化、高效的知识蒸馏框架。

Method: 提出HPM-KD框架，包含六个协同组件：1) 基于元学习的自适应配置管理器，消除手动超参数调优；2) 渐进蒸馏链，自动确定中间模型；3) 注意力加权多教师集成，学习动态的每样本权重；4) 元学习温度调度器，在训练过程中自适应调整温度；5) 并行处理管道，智能负载均衡；6) 共享优化内存，跨实验重用。

Result: 在CIFAR-10、CIFAR-100和表格数据集上的实验表明：HPM-KD实现10-15倍压缩同时保持85%准确率保留，消除了手动调优需求，通过并行化减少30-40%训练时间。消融研究确认每个组件的独立贡献（0.10-0.98个百分点）。

Conclusion: HPM-KD是一个全面解决知识蒸馏关键限制的框架，通过六个协同组件实现了自动化、高效的知识蒸馏，显著提高了压缩效率和训练速度，已作为开源DeepBridge库的一部分提供。
...
### [70] [Analysis of Dirichlet Energies as Over-smoothing Measures](https://arxiv.org/abs/2512.09890)
*Anna Bison,Alessandro Sperduti*

Main category: cs.LG

TL;DR: 本文分析了非归一化图拉普拉斯和归一化图拉普拉斯诱导的Dirichlet能量之间的区别，指出后者不符合节点相似性度量的公理化定义，并形式化了两种定义的基本谱特性以指导GNN架构中度量选择。


<details>
  <summary>Details</summary>
Motivation: 在GNN训练中，过平滑是一个关键问题，通常使用Dirichlet能量作为度量。然而，非归一化和归一化图拉普拉斯诱导的Dirichlet能量之间存在混淆，导致监控动态时产生歧义。需要澄清这两种度量的区别，并确定哪种在谱上与GNN架构兼容。

Method: 1. 分析两种Dirichlet能量定义的区别；2. 证明归一化图拉普拉斯诱导的Dirichlet能量不符合Rusch等人提出的节点相似性度量的公理化定义；3. 形式化两种定义的基本谱特性；4. 基于谱兼容性为GNN架构选择合适度量。

Result: 归一化图拉普拉斯诱导的Dirichlet能量不满足节点相似性度量的公理化要求，而两种定义在谱特性上存在关键差异。通过形式化这些谱特性，可以明确选择与GNN架构谱兼容的度量，从而消除监控过平滑动态时的歧义。

Conclusion: 选择正确的Dirichlet能量定义对于准确监控GNN中的过平滑现象至关重要。非归一化图拉普拉斯诱导的Dirichlet能量在谱特性上更适合作为节点相似性度量，而归一化版本存在理论缺陷。这一分析为GNN训练中过平滑监控提供了明确的指导原则。
...
### [71] [Exploring Protein Language Model Architecture-Induced Biases for Antibody Comprehension](https://arxiv.org/abs/2512.09894)
*Mengren,Liu,Yixiang Zhang,Yiming,Zhang*

Main category: cs.LG

TL;DR: 本文系统评估了不同蛋白质语言模型架构在理解抗体序列特性和功能方面的差异，发现抗体特异性模型能自然学习关注CDR区域，而通用蛋白质模型需要明确的CDR训练策略。


<details>
  <summary>Details</summary>
Motivation: 尽管蛋白质语言模型在理解蛋白质序列方面取得了显著进展，但不同模型架构在捕捉抗体特异性生物学特性方面的能力差异尚未被系统研究。

Method: 评估了三种最先进的蛋白质语言模型（AntiBERTa、BioBERT和ESM2），以通用语言模型GPT-2为基线，在抗体靶标特异性预测任务上进行对比。通过注意力归因分析，研究了不同模型对生物学特征的捕捉能力。

Result: 所有蛋白质语言模型都实现了高分类准确率，但在捕捉V基因使用、体细胞超突变模式和同种型信息等生物学特征方面表现出不同的偏差。抗体特异性模型如AntiBERTa能自然学习关注互补决定区（CDRs），而通用蛋白质模型通过明确的CDR聚焦训练策略能显著受益。

Conclusion: 研究揭示了模型架构与生物学特征提取之间的关系，为未来计算抗体设计中的蛋白质语言模型开发提供了有价值的指导。
...
### [72] [Closing the Train-Test Gap in World Models for Gradient-Based Planning](https://arxiv.org/abs/2512.09929)
*Arjun Parthasarathy,Nimit Kalra,Rohun Agrawal,Yann LeCun,Oumayma Bounou,Pavel Izmailov,Micah Goldblum*

Main category: cs.LG

TL;DR: 提出改进世界模型训练方法，通过缩小训练与测试目标差距，实现高效的基于梯度的规划，在10%时间预算下达到或超越传统无梯度方法性能


<details>
  <summary>Details</summary>
Motivation: 基于世界模型和模型预测控制的方法可以从专家轨迹数据中学习并泛化到多种规划任务，但现有的基于梯度规划方法性能落后于其他方法。研究发现世界模型训练时使用下一状态预测目标，而测试时用于估计动作序列，存在训练-测试差距

Method: 提出训练时数据合成技术来缩小训练-测试差距，改进世界模型训练方法，使基于梯度的规划更加高效。具体包括针对梯度规划优化的训练策略

Result: 在多种物体操作和导航任务中，该方法在仅使用10%时间预算的情况下，性能达到或超越了传统的无梯度交叉熵方法（CEM）

Conclusion: 通过缩小世界模型训练与测试目标之间的差距，可以有效提升基于梯度规划的性能，实现高效且性能优越的模型预测控制
...
### [73] [Calibrated Trust in Dealing with LLM Hallucinations: A Qualitative Study](https://arxiv.org/abs/2512.09088)
*Adrian Ryser,Florian Allwein,Tim Schlippe*

Main category: cs.AI

TL;DR: 本文研究大型语言模型幻觉如何影响用户信任，发现幻觉不会导致全面不信任，而是引发情境敏感的信任校准，并识别直觉作为新的用户相关信任因素。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型产生的幻觉（事实错误但看似合理）如何影响用户对LLM的信任以及用户与LLM的互动，探索日常使用中的信任动态。

Method: 采用定性研究方法，对192名参与者进行研究，基于Lee & See的校准信任模型和Afroogh等人的信任相关因素框架进行分析。

Result: 发现幻觉不会导致全面不信任，而是引发情境敏感的信任校准；确认期望、先前经验、用户专业知识为信任因素，识别直觉作为新的用户相关因素；信任动态还受感知风险和决策风险等情境因素影响。

Conclusion: 验证了Blöbaum提出的递归信任校准过程，并通过纳入直觉作为用户相关信任因素进行扩展，基于研究结果提出了负责任和反思性LLM使用的实用建议。
...
### [74] [AI TIPS 2.0: A Comprehensive Framework for Operationalizing AI Governance](https://arxiv.org/abs/2512.09114)
*Pamela Gupta*

Main category: cs.AI

TL;DR: 论文提出AI TIPS框架2.0版本，解决当前AI治理框架的三个关键缺陷：缺乏针对具体用例的风险评估、原则性指导缺乏可操作控制措施、以及规模化实施治理的机制缺失。


<details>
  <summary>Details</summary>
Motivation: 当前AI治理框架存在三个关键问题：1) 缺乏针对具体用例的风险评估能力，如Humana集体诉讼案所示；2) 现有框架(如ISO 42001和NIST AI RMF)停留在概念层面，缺乏可操作控制措施；3) 组织缺乏规模化实施治理的机制，无法将可信AI实践嵌入整个开发生命周期。

Method: 提出AI TIPS(人工智能信任集成支柱可持续性)2.0框架，这是对2019年开发的全面操作框架的更新，比NIST AI风险管理框架早四年。该框架直接针对上述三个挑战提供解决方案。

Result: AI TIPS框架提供了：1) 针对具体用例的定制化风险评估方法；2) 将治理要求转化为具体技术实施的可操作控制措施；3) 规模化实施治理的机制，包括在整个开发生命周期嵌入可信AI实践、量化合规性测量以及为从董事会到数据科学家的不同角色提供适当可见性。
...
### [75] [A Categorical Analysis of Large Language Models and Why LLMs Circumvent the Symbol Grounding Problem](https://arxiv.org/abs/2512.09117)
*Luciano Floridi,Yiyang Jia,Fernando Tohmé*

Main category: cs.AI

TL;DR: 论文提出了一个形式化的范畴论框架，分析人类和大型语言模型如何将内容转化为关于可能世界状态空间W的真值命题，论证LLMs并非解决而是绕过了符号接地问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析人类和大型语言模型在将内容转化为真值命题方面的根本差异，特别是探讨LLMs是否真正解决了符号接地问题，还是仅仅绕过了这个问题。

Method: 采用形式化的范畴论框架，建立数学模型来分析人类和LLMs如何将内容映射到可能世界状态空间W的真值命题，通过范畴结构对比两者的语义处理机制。

Result: 分析结果表明，大型语言模型并没有真正解决符号接地问题，而是通过不同的机制绕过了这个问题，与人类处理语义的方式存在根本性差异。

Conclusion: 结论是LLMs并非解决符号接地问题，而是通过范畴论框架揭示的机制规避了这个问题，这对理解AI语义处理能力和局限性具有重要意义。
...
### [76] [SDialog: A Python Toolkit for End-to-End Agent Building, User Simulation, Dialog Generation, and Evaluation](https://arxiv.org/abs/2512.09142)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个统一的框架来系统性地构建、评估和理解基于大语言模型的对话系统，需要将对话生成、评估和机制可解释性整合到一个完整的工具包中。

Method: 基于标准化的Dialog表示，提供：1）人物驱动的多代理模拟与可组合编排；2）结合语言指标、LLM作为评判者和功能正确性验证器的综合评估；3）通过特征消融和诱导进行激活检查和引导的机制可解释性工具；4）包含3D房间建模和麦克风效果的完整声学模拟音频生成。

Result: SDialog工具包成功集成了所有主要LLM后端，支持统一API下的混合后端实验，为研究人员提供了系统构建、基准测试和理解对话系统的完整解决方案。
...
### [77] [Visual Categorization Across Minds and Models: Cognitive Analysis of Human Labeling and Neuro-Symbolic Integration](https://arxiv.org/abs/2512.09340)
*Chethana Prasad Kabgere*

Main category: cs.AI

TL;DR: 该研究对比人类与AI系统在模糊视觉刺激下的图像标注表现，分析两者在感知、推理和决策上的异同，为构建神经符号AI架构提供认知科学基础。
...
Method: 结合计算认知科学、认知架构和连接主义-符号混合模型，对比人类策略（类比推理、形状识别、置信度调节）与AI的特征处理。基于Marr的三层次假设、Simon的有限理性和Thagard的表征情感框架，分析参与者反应与Grad-CAM可视化模型注意力的关系，并通过ACT-R和Soar认知架构解释人类行为。

Result: 研究发现人类与AI系统在低分辨率、感知退化刺激下的图像标注表现存在关键相似点和差异。人类采用分层启发式决策策略，而AI主要依赖特征处理。Grad-CAM可视化揭示了模型注意力模式与人类认知策略的对比。

Conclusion: 研究为未来神经符号架构提供了理论基础，这类架构将结构化符号推理与连接主义表征相结合，受具身性、可解释性和认知对齐原则指导，有望开发出不仅性能优越，而且可解释且具有认知基础的AI系统。
...
### [78] [Architectures for Building Agentic AI](https://arxiv.org/abs/2512.09458)
*Sławomir Nowaczyk*

Main category: cs.AI

TL;DR: 本文认为AI代理系统的可靠性主要是一个架构属性，通过组件化、接口规范和显式控制循环来实现，提出了实用的代理分类和可靠性设计指南。


<details>
  <summary>Details</summary>
Motivation: 探讨如何构建可靠的代理式和生成式AI系统，强调可靠性不是偶然特性而是需要精心设计的架构属性，旨在为AI代理系统的设计和实现提供系统性指导。

Method: 定义代理系统为在闭环中运行的目标导向、工具使用的决策者，通过原则性组件化（目标管理器、规划器、工具路由器、执行器、内存、验证器、安全监控器、遥测）、规范接口（模式约束、验证、最小权限工具调用）和显式控制与保障循环来构建可靠性。

Result: 提出了实用的代理分类法（工具使用代理、内存增强代理、规划与自我改进代理、多代理系统、具身或网络代理），分析了每种模式如何重塑可靠性范围和故障模式，提炼了类型化模式、幂等性、权限管理、事务语义、内存来源与卫生、运行时治理和模拟执行前保障等设计指南。

Conclusion: AI代理系统的可靠性主要源于精心设计的架构，通过系统性的组件化、接口规范和显式控制循环可以实现可靠的代理行为，为实际AI系统开发提供了可操作的设计原则和模式。
...
### [79] [Toward Closed-loop Molecular Discovery via Language Model, Property Alignment and Strategic Search](https://arxiv.org/abs/2512.09566)
*Junkai Ji,Zhangfan Yang,Dong Xu,Ruibin Bai,Jianqiang Li,Tingjun Hou,Zexuan Zhu*

Main category: cs.AI

TL;DR: Trio是一个整合了片段分子语言建模、强化学习和蒙特卡洛树搜索的分子生成框架，用于实现有效且可解释的闭环靶向分子设计，在结合亲和力、类药性和合成可行性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统药物发现方法耗时昂贵且成功率低，现有的生成模型存在泛化能力不足、可解释性有限、过度强调结合亲和力而忽视关键药理学性质等问题，限制了其实际应用价值。

Method: Trio框架整合三个关键组件：1）基于片段的分子语言建模实现上下文感知的片段组装；2）强化学习确保物理化学和合成可行性；3）蒙特卡洛树搜索平衡新颖化学型探索和蛋白质结合口袋中有前景中间体的利用。

Result: Trio可靠地生成化学有效且药理学增强的配体，在结合亲和力（+7.85%）、类药性（+11.10%）和合成可行性（+12.05%）方面优于最先进方法，同时分子多样性扩展超过四倍。

Conclusion: Trio框架通过整合片段建模、强化学习和树搜索，实现了有效、可解释的闭环靶向分子设计，在多个关键药理学指标上显著优于现有方法，为药物发现提供了更实用的生成工具。
...
### [80] [An End-to-end Planning Framework with Agentic LLMs and PDDL](https://arxiv.org/abs/2512.09629)
*Emanuele La Malfa,Ping Zhu,Samuele Marro,Sara Bernardini,Michael Wooldridge*

Main category: cs.AI

TL;DR: 提出一个由LLM驱动的端到端规划框架，通过验证器支持，能够将自然语言规范自动转换为PDDL模型并生成可执行计划，最后翻译回自然语言以提高可读性。


<details>
  <summary>Details</summary>
Motivation: 解决传统规划系统中需要人工干预将自然语言需求转换为形式化规划模型的问题，利用LLM实现完全自动化的端到端规划流程，同时处理规划中的时间约束、最优性要求以及规范中的模糊和矛盾。

Method: 使用编排器接收自然语言规范，通过多个LLM驱动的子模块（代理）迭代精炼PDDL领域和问题模型，处理规划需求如时间约束和最优性，解决规范中的模糊和矛盾，然后传递给外部规划引擎生成计划，最后将计划翻译回自然语言。

Result: 框架在多个领域和任务中表现出灵活性和有效性，包括Google NaturalPlan基准测试和PlanBench，以及Blocksworld和汉诺塔等规划问题（这些领域LLM通常难以处理）。框架可与任何PDDL规划引擎和验证器集成。

Conclusion: 该框架代表了LLM辅助端到端规划的重要进展，能够完全自动化地将自然语言规范转换为可执行计划，无需人工干预，具有广泛的适用性和集成能力。
...
### [81] [Gaussian Process Aggregation for Root-Parallel Monte Carlo Tree Search with Continuous Actions](https://arxiv.org/abs/2512.09727)
...
<details>
  <summary>Details</summary>
Motivation: 在连续动作空间中，当计算时间有限但需要最佳性能时，如何有效聚合不同线程的统计信息是一个重要但尚未充分探索的问题。现有的根并行MCTS变体在连续动作空间中缺乏有效的统计聚合方法。

Method: 使用高斯过程回归来获取未在环境中试验过的有希望动作的价值估计。该方法通过回归模型从已试验动作的统计信息中推断未试验动作的价值，从而更有效地聚合多线程的探索结果。

Result: 在6个不同领域进行了系统评估，结果表明该方法优于现有的聚合策略，同时只需要适度的推理时间增加。

Conclusion: 提出的基于高斯过程回归的统计聚合方法在连续动作空间的根并行MCTS中表现优异，为在线规划提供了更有效的多线程统计信息整合方案。
...
### [82] [Analyzing Planner Design Trade-offs for MAPF under Realistic Simulation](https://arxiv.org/abs/2512.09736)
*Jingtian Yan,Zhifei Li,William Kang,Stephen F. Smith,Jiaoyang Li*

Main category: cs.AI

TL;DR: 该研究系统分析了多智能体路径规划算法在实际执行环境中的性能影响因素，重点关注解决方案最优性、动力学模型准确性以及两者交互作用对实际部署效果的影响。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体路径规划算法评估框架通常基于简化的机器人模型，导致算法基准测试与实际性能之间存在显著差距。虽然SMART等框架引入了动力学建模，但规划器设计选择对实际执行性能的影响尚未得到系统研究。

Method: 在SMART框架基础上，系统研究三个关键因素：1）解决方案最优性与执行性能的关系；2）系统性能对动力学模型不准确性的敏感性；3）模型准确性与规划最优性之间的相互作用。通过实证方法分析这些设计选择在实际场景中的影响。

Result: 研究发现规划器设计选择对实际执行性能有显著影响，揭示了算法最优性与实际部署效果之间的复杂关系，以及动力学模型准确性对系统性能的关键作用。

Conclusion: 该研究为多智能体路径规划社区提供了向实际部署方向发展的指导，强调了在实际约束下考虑规划器设计选择的重要性，并指出了未来研究的关键挑战和方向。
...
### [83] [RIFT: A Scalable Methodology for LLM Accelerator Fault Assessment using Reinforcement Learning](https://arxiv.org/abs/2512.09829)
...
<details>
  <summary>Details</summary>
Motivation: 现代AI加速器规模巨大，传统故障评估方法面临计算成本过高、关键故障模式覆盖率不足的问题，需要一种可扩展的自动化故障发现框架。

Method: RIFT将最坏情况故障搜索转化为序列决策问题，结合混合灵敏度分析进行搜索空间剪枝，使用强化学习智能生成最小化、高影响力的测试套件。

Result: 在NVIDIA A100 GPU上的十亿参数大语言模型工作负载评估中，RIFT相比进化方法实现2.2倍加速，相比随机故障注入减少99%测试向量，同时获得更优的故障覆盖率。RIFT指导的选择性纠错码比均匀三模冗余保护的成本效益提高12.8倍。

Conclusion: RIFT提供了一个可扩展的自动化故障评估框架，能够高效发现高影响力故障场景，生成可直接集成到商业RTL验证流程的UVM兼容验证工件，支持智能硬件保护策略。
...
### [84] [Interpretation as Linear Transformation: A Cognitive-Geometric Model of Belief and Meaning](https://arxiv.org/abs/2512.09831)
*Chainarong Amornbunchornvej*

Main category: cs.AI

TL;DR: 该论文提出了一个几何框架来建模认知异质智能体之间的信念、动机和影响，将智能体表示为个性化价值空间，信念作为结构化向量，其传播受线性解释映射调节，并提出了"无零空间领导条件"等核心概念。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决认知异质智能体之间信念传播、动机变化和相互影响的建模问题，为理解信念如何在多样化认知系统中传播、变异或消失提供理论基础，统一概念空间、社会认识论和AI价值对齐等领域的见解。

Method: 采用几何框架建模：每个智能体表示为个性化价值空间（向量空间），信念形式化为结构化向量，信念传播通过线性解释映射调节，信念存活需要避免映射的零空间，从而建立信念可理解性、误传和信念消亡的结构标准。

Result: 证明了信念扭曲、动机漂移、反事实评估和相互理解限制都源于纯代数约束；提出了核心结果"无零空间领导条件"，将领导力特征化为表征可达性而非说服或权威；解释了抽象实体如何在多样化认知几何中传播、变异或消失。

Conclusion: 该认知几何视角通过将意义保存建立在结构兼容性而非共享信息或理性基础上，阐明了人类和人工系统中影响的认知边界，为分析异质智能体间的信念动态提供了通用基础框架。
...
### [85] [Comparing AI Agents to Cybersecurity Professionals in Real-World Penetration Testing](https://arxiv.org/abs/2512.09882)
...
TL;DR: AI安全代理ARTEMIS在企业网络环境中与人类网络安全专家进行首次全面对比评估，在8000台主机的大型网络中表现优异，排名第二，超越9/10的人类参与者。


<details>
  <summary>Details</summary>
Motivation: 首次在真实企业环境中全面评估AI代理与人类网络安全专业人员的性能差异，探索AI在网络安全领域的实际应用潜力和局限性。

Method: 在包含约8000台主机、12个子网的大型大学网络中，评估10名网络安全专业人员、6个现有AI代理以及新开发的ARTEMIS多代理框架。ARTEMIS具有动态提示生成、任意子代理和自动漏洞分类功能。

Result: ARTEMIS总体排名第二，发现9个有效漏洞，有效提交率达82%，超越9/10的人类参与者。现有框架如Codex和CyAgent表现不如大多数人类参与者。AI代理在系统枚举、并行利用和成本方面具有优势（ARTEMIS变体成本18美元/小时 vs 专业渗透测试人员60美元/小时），但存在误报率高和GUI任务处理困难等能力差距。

Conclusion: AI安全代理在特定网络安全任务中已展现出与顶尖人类专家相当的技术水平和提交质量，具有成本效益和并行处理优势，但需要解决误报率高和GUI交互等能力差距才能实现更广泛的实际应用。
...
### [86] [Human-in-the-Loop and AI: Crowdsourcing Metadata Vocabulary for Materials Science](https://arxiv.org/abs/2512.09895)
...
TL;DR: MatSci-YAMZ平台结合AI和众包人机协同，支持材料科学领域元数据词汇开发，验证了AI-HILT模型的可行性。
...
Method: 开发MatSci-YAMZ平台，集成人工智能和人类参与循环（包括众包），在材料科学领域进行概念验证，6名参与者通过提供术语定义和示例来优化AI生成的定义。

Result: 成功创建19个AI生成的定义，迭代反馈循环验证了AI-HILT精炼的可行性，证明该模型能够增强语义透明度并减少共识构建时间。

Conclusion: AI-HILT模型可行且符合FAIR和开放科学原则，为跨领域扩展提供了研究协议，具有增强语义透明度和加速元数据词汇开发的潜力。
...
### [87] [SCOPE: Language Models as One-Time Teacher for Hierarchical Planning in Text Environments](https://arxiv.org/abs/2512.09897)
...
<details>
  <summary>Details</summary>
Motivation: 解决基于文本环境中长期规划的挑战：开放的动作空间、模糊观察和稀疏反馈。现有方法依赖训练和推理期间频繁查询LLM，计算成本高且难以高效部署，且使用未经调整的预训练LLM参数固定，无法适应目标任务

Method: SCOPE（子目标条件预训练）是一种一次性分层规划器，仅在初始化时使用LLM生成的子目标预训练轻量级学生模型。与先前方法不同，SCOPE直接从示例轨迹中推导子目标，避免了重复的LLM查询

Result: 在TextCraft环境中，相比LLM分层代理ADaPT（成功率0.52），SCOPE达到0.56成功率，推理时间从164.4秒大幅减少到仅3.0秒

Conclusion: 尽管LLM生成的子目标可能不是最优的，但可以作为文本规划任务中分层目标分解的良好起点。SCOPE在保持性能的同时显著提高了效率，为高效文本规划提供了实用解决方案
...
### [88] [Bayesian Networks, Markov Networks, Moralisation, Triangulation: a Categorical Perspective](https://arxiv.org/abs/2512.09908)
*Antonio Lorenzin,Fabio Zanasi*

Main category: cs.AI

TL;DR: 提出一个范畴论框架，将贝叶斯网络和马尔可夫网络之间的道德化和三角化转换建模为函子，并重新解释变量消除算法为函子。


<details>
  <summary>Details</summary>
Motivation: 为概率图模型中的道德化和三角化转换建立一个形式化的范畴论框架，以区分语法和语义层面的修改，并提供更结构化的理论视角。

Method: 将贝叶斯网络和马尔可夫网络表示为从"语法"域到"语义"域的函子，将道德化和三角化建模为网络类别之间的函子，并通过函子预组合在语法上归纳定义。

Result: 建立了一个范畴论框架，其中道德化是纯语法的，三角化依赖于语义，并将变量消除算法重新解释为将三角化过程分为纯语法和纯语义两部分的函子。

Conclusion: 该工作为概率图模型理论引入了函子视角，突出了语法和语义修改之间的区别，为图模型转换提供了更形式化的理论基础。
