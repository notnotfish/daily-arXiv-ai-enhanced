...
TL;DR: TOGGLE提出了一种基于时序逻辑的LLM压缩框架，使用信号时序逻辑（STL）形式化指定和强制执行语言属性，通过STL鲁棒性引导的贝叶斯优化探索分层量化和剪枝配置，在无需重训练或微调的情况下生成满足语言约束的压缩模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在自然语言任务上表现出色，但需要大量计算资源，限制了其在资源受限的边缘设备上的部署。现有的压缩技术（如量化和剪枝）通常会损害关键的语言特性，并且缺乏保持模型行为的正式保证。

Method: TOGGLE框架利用信号时序逻辑（STL）形式化指定和强制执行语言属性。采用STL鲁棒性引导的贝叶斯优化来系统探索分层量化和剪枝配置，生成满足指定语言约束的压缩模型，无需重训练或微调。

Result: 在四种LLM架构（GPT-2、DeepSeek-V2 7B、LLaMA 3 8B和Mistral 7B）上评估TOGGLE，实现了计算成本（FLOPs）最高3.3倍的减少，模型大小最高68.8%的减少，同时满足所有语言属性。
...
TL;DR: 论文提出"拼凑式AGI"假说，认为通用智能可能首先通过多个子AGI智能体协调实现，而非单一AGI系统，因此需要新的分布式AGI安全框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全和对齐研究主要关注单一AGI系统的安全保障，忽视了多个子AGI智能体通过协调合作实现通用能力的可能性。随着具有工具使用能力和协调能力的AI智能体快速部署，这种"拼凑式AGI"假说需要认真考虑并开发相应的安全保障措施。

Method: 提出分布式AGI安全框架，核心是设计和实施虚拟智能体沙盒经济系统（不可渗透或半渗透），通过稳健的市场机制管理智能体间交易，结合适当的可审计性、声誉管理和监督机制来缓解集体风险。

Result: 论文提出了一个超越个体智能体评估和对齐的新安全框架，重点关注智能体群体协调带来的集体风险，为应对"拼凑式AGI"场景提供了系统性的安全解决方案。

Conclusion: "拼凑式AGI"假说需要得到重视，应开发相应的安全保障和缓解措施。提出的分布式AGI安全框架通过虚拟智能体沙盒经济和市场机制，为管理多个智能体协调带来的集体风险提供了可行路径。
...
TL;DR: 本文提出了社会责任感堆栈（SRS），这是一个六层架构框架，将社会价值观嵌入AI系统作为显式约束、保障措施、行为接口、审计机制和治理流程，将责任建模为对技术社会系统的闭环监督控制问题。


<details>
  <summary>Details</summary>
Motivation: 当前负责任AI和治理工作提供了重要的规范性原则，但往往缺乏在整个系统生命周期中可执行的工程机制。需要将社会价值观实际嵌入AI系统，提供可操作的工程和运营控制。

Method: 引入社会责任感堆栈（SRS）六层架构框架：1）约束层，2）保障层，3）行为接口层，4）审计层，5）治理层，6）制度层。采用基于约束的统一表述，引入安全包络和反馈解释，将责任建模为对技术社会系统的闭环监督控制问题。

Result: SRS框架能够将规范性目标转化为可操作的工程和运营控制，通过临床决策支持、协作自动驾驶车辆和公共部门系统的案例研究展示了其实际应用。该框架实现了公平性、自主性、认知负担和解释质量的持续监控和执行。

Conclusion: SRS框架连接了伦理学、控制理论和AI治理，为负责任、适应性强且可审计的技术社会AI系统提供了实用基础，解决了现有方法缺乏可执行工程机制的问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管具有显式推理能力的大型语言模型在数学推理方面表现出色，但仍存在过程错误，如计算错误、逻辑脆弱和表面合理但无效的步骤

Method: 提出Generative Adversarial Reasoner框架：1) 使用计算高效的审查计划将推理链划分为逻辑完整的片段；2) 判别器评估每个片段的合理性并提供结构化理由；3) 通过对抗强化学习联合训练推理器和判别器，获得密集的步骤级奖励信号

Result: 在多个数学基准测试中取得一致改进：AIME24上，DeepSeek-R1-Distill-Qwen-7B从54.0提升到61.3(+7.3)，DeepSeek-R1-Distill-Llama-8B从43.7提升到53.7(+10.0)。模块化判别器还支持灵活的奖励塑造

Conclusion: Generative Adversarial Reasoner通过对抗强化学习提供密集、校准良好的步骤级奖励，改善信用分配、提高样本效率，显著提升LLM的推理质量，并支持多种应用场景
...
<details>
  <summary>Details</summary>
Motivation: 罕见病如胶原VI相关肌营养不良症（COL6-RD）的诊断面临数据稀缺和碎片化问题，跨机构数据共享存在隐私、法规和物流障碍，需要一种既能保护隐私又能实现协作学习的方法。

Method: 采用Sherpa.ai联邦学习平台，在两个国际组织的分布式数据集上进行协作训练，使用患者来源成纤维细胞培养的胶原VI免疫荧光显微镜图像，训练机器学习模型对COL6-RD的三种主要致病机制进行分类。

Result: 联邦学习模型在胶原VI患者图像分类任务中取得了0.82的F1分数，显著优于单机构模型（0.57-0.75），证明联邦学习能大幅提升诊断效用和泛化能力。

Conclusion: 联邦学习为罕见病诊断提供了有效的隐私保护协作解决方案，不仅能提高诊断准确性，还有助于解释意义未明变异和指导测序策略优化，推动新致病变异的发现。
